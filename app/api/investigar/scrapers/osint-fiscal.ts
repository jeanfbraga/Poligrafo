...
import { transparenciaLimiter } from "../../../../lib/services/rate-limiter";
import { buscarProcessosDataJud } from "./judiciario";
import { listarAtividadesAuditoria } from "../../../../lib/denasus/client";
import { buscarInabilitadosTCU, buscarCadirregTCU } from "../../../../lib/tcu/client";
import { traduzirJuridiquesSancoes } from "../ai_helpers";

export async function investigarPolitico(cpfLimpo: string, nome: string, uf: string, pessoaId: string, sendEvent: any) {
    let patrimonioTotal = 0;
    let sancoesCgu = false;
    let alertasPessoais: string[] = [];
    let bensDeclarados: any[] = [];

    if (!cpfLimpo || cpfLimpo === "00000000000") return { patrimonioTotal, sancoesCgu, alertasPessoais, bensDeclarados };
    ...
    try {
        ...
        if (resBusca.ok) {
            ...
            if (candidato) {
                ...
                if (resBens.ok) {
                    ...
                    if (patrimonioTotal > 0) alertasPessoais.push(`[TSE] Patrimônio Declarado (2022): R$ ${patrimonioTotal.toLocaleString('pt-BR')}`);
                }
            }
        }
        ...
        if (apiKey) {
            ...
            const pSancao = cpfLimpo && cpfLimpo !== "00000000000" && cpfLimpo.length === 11 
                ? `codigoSancionado=${cpfLimpo}` 
                : `nomeSancionado=${encodeURIComponent(nome)}`;
            const pCeaf = cpfLimpo && cpfLimpo !== "00000000000" && cpfLimpo.length === 11 
            ...
            const pPep = cpfLimpo && cpfLimpo !== "00000000000" && cpfLimpo.length === 11
                ? `cpf=${cpfLimpo}`
                : `nome=${encodeURIComponent(nome)}`;

            const consultasSancoesPEP = [
                ...basesSancoes.map(b => fetchWithTimeout(b.url, { headers, timeout: 5000 })
                    .then(async r => ({ key: b.key, nome: b.nome, data: r.ok ? await r.json() : [] }))
                    .catch(() => ({ key: b.key, nome: b.nome, data: [] }))
                ),
                fetchWithTimeout(`${BASE_TRANSPARENCIA}/peps?${pPep}&pagina=1`, { headers, timeout: 5000 })
                    .then(async r => ({ key: 'pep', nome: 'PEP', data: r.ok ? await r.json() : [] }))
                    .catch(() => ({ key: 'pep', nome: 'PEP', data: [] }))
            ];

            const resultadosSancoesPEP = await Promise.allSettled(consultasSancoesPEP);

            for (const res of resultadosSancoesPEP) {
                if (res.status !== 'fulfilled') continue;
                const { key, nome, data } = res.value as { key: string; nome: string; data: any };
        ...
        await buscarProcessosDataJud(cpfLimpo, uf, pessoaId, sendEvent, alertasPessoais);
            ...
            try {
                ...
                if (resConvenios.ok) {
                    ...
                    if (Array.isArray(conveniosData) && conveniosData.length > 0) {
                        const valorTotalConvenio = conveniosData.reduce((acc: number, c: any) => acc + (Number(c.valorTotal || c.valor_global || 0)), 0);
                        const orgaoConcedente = conveniosData[0]?.concedente?.nome || conveniosData[0]?.orgaoConcedente || 'Órgão Federal';
                        alertasPessoais.push(`[TRANSPARÊNCIA] Cidadão figura como convenente em ${conveniosData.length} Convênio(s) Federal(is). Valor total: R$ ${valorTotalConvenio.toLocaleString('pt-BR')}`);
                        sendEvent('NODE_NOVO', {
                            ...
                            data: {
                                ...
                                situacao: conveniosData[0]?.situacao?.descricao || conveniosData[0]?.situacao || 'Vigente',
                                numeroConvenio: conveniosData[0]?.numeroConvenio || conveniosData[0]?.numero || 'N/A',
                                score_letalidade: 50,
                ...
                console.warn("[TRANSPARÊNCIA] Erro ao buscar convênios por CPF:", err.message);
            }
        }
...
export async function buscarReceitasFederais(docLimpo: string, pessoaId: string, sendEvent: any) {
    ...
    try {
        ...
        const res = await fetchWithTimeout(`https://api.portaldatransparencia.gov.br/api-de-dados/despesas/por-favorecido?${paramCgu}`, { headers: { 'chave-api-dados': apiKey }, timeout: 8000 });
        if (res.ok) {
    ...
    } catch (e) { console.error("Erro CGU:", e); }

    if (isCnpj) {
        try {
            const res = await fetchWithTimeout(`https://compras.dados.gov.br/contratos/v1/contratos.json?cnpj_contratada=${docLimpo}`, { timeout: 5000 });
            if (res.ok) {
...
export async function buscarCartaoCorporativo(cpfLimpo: string, pessoaId: string, sendEvent: any, casaPolitico?: string) {
    ...
    try {
        ...
        if ((!data || data.length === 0) && (casaPolitico === 'PRESIDENCIA_DA_REPUBLICA' || cpfLimpo === '76463456868' || pessoaId.includes('lula') || pessoaId.includes('bolsonaro') || pessoaId.includes('presidente'))) {
            ...
            data.slice(0, 10).forEach((item: any, idx: number) => {
                ...
                sendEvent('NODE_NOVO', {
                    ...
                    data: {
                        ...
                        orgao: item.unidadeGestora?.orgaoVinculado?.nomeOrgao || 'Órgão Federal',
    ...
    } catch (e) { console.error("[OSINT CPGF Error]", e); }
}

export async function buscarViagensFAB(cpfLimpo: string, pessoaId: string, sendEvent: any, casaPolitico?: string) {
    if (!cpfLimpo || cpfLimpo === "00000000000") return;
    const apiKey = process.env.TRANSPARENCIA_API_KEY || '';
    if (!apiKey) return;
    try {
        ...
        if (cpfLimpo.length === 11) {
            const res = await fetchWithTimeout(url, { headers: { 'chave-api-dados': apiKey }, timeout: 6000 });
            if (res.ok) {
                data = await res.json();
            }
        }
        ...
        if (Array.isArray(data) && data.length > 0) {
            sendEvent('STATUS', { msg: `[VIAGENS] Rastreando diárias governamentais e voos da Força Aérea Brasileira (FAB).` });
    ...
    } catch (e) { console.error("[OSINT Viagens Error]", e); }
}
