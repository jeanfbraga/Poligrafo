...
export async function buscarProxyOsint(
    ...
    const cpfParaSancoes = isCnpj ? null : docLimpo;
    const cnpjParaEmpresas = isCnpj ? docLimpo : (cnpjOpcional ? String(cnpjOpcional).replace(/\D/g, '') : null);
    const apiKey = process.env.TRANSPARENCIA_API_KEY || '';

    console.log(`[PROXY OSINT] Iniciando varredura. Principal: ${docLimpo}. CNPJ Secundário: ${cnpjParaEmpresas || 'Nenhum'}`);
            ...
            fetchWithTimeout(
                ...
                items.slice(0, 20).forEach((item: any) => {
                    despesasFederais.push({
                        ...
                        nomeFornecedor: item.nomeFavorecido || item.nomeCredor || nomeVereador || 'N/A',
                        tipoDespesa: item.funcao || item.elementoDespesa || 'Despesa Federal',
                ...
                if (items.length > 0) partes.push(`${items.length} pagamentos federais localizados`);
            }).catch(() => { })
        );
            ...
            promessas.push(
                fetchWithTimeout(
                    ...
                    contratos.slice(0, 10).forEach((c: any) => {
                        despesasFederais.push({
                            cnpjCpfFornecedor: docLimpo,
                            nomeFornecedor: c.fornecedor?.nome || nomeVereador || 'Contrato Federal',
                    ...
                    if (contratos.length > 0) partes.push(`${contratos.length} contratos federais`);
                }).catch(() => { })
            );
        }
    }
    ...
    if (cnpjParaEmpresas) {
        promessas.push(
            fetchWithTimeout(`https://brasilapi.com.br/api/cnpj/v1/${cnpjParaEmpresas}`, { timeout: 6000 })
                .then(async (res) => {
                    if (!res.ok) return;
    ...
    if (partes.length > 0) {
        ...
        statusMensagem = 'API Municipal offline/fechada. Varredura Federal e Societária concluída sem achados.';
    }

    console.log(`[PROXY OSINT] Resultado: ${despesasFederais.length} despesas, ${empresasAssociadas.length} empresas. ${statusMensagem}`);
