...
async function buscarContratosComoOrgao(cnpj: string, yearsToFetch = 2) {
    ...
    for (let year = currentYear; year > currentYear - yearsToFetch; year--) {
        ...
        try {
            ...
            console.error(`[PNCP] Erro ao buscar contratos como orgão no ano ${year}`, e);
        }
    }
...
export async function GET(request: Request) {
    ...
    try {
        ...
        const todosContratos = [...comoOrgao, ...formatadosFornecedor].sort((a, b) => {
            const dataA = a.data || '';
            const dataB = b.data || '';
        ...
        return NextResponse.json({ error: 'Falha ao buscar contratos no PNCP' }, { status: 500 });
    }
}
