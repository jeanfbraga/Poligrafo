...
export async function GET(req: NextRequest) {
    ...
    if (!nome) {
        return NextResponse.json({ error: 'Nome do vereador é obrigatório' }, { status: 400 });
    }
    ...
    try {
        ...
        const gastosPorCategoria = Object.keys(categoriaMap).map(k => ({
            categoria: k,
            valor: categoriaMap[k]
        })).sort((a, b) => b.valor - a.valor);

        const topFornecedores = Object.keys(fornecedorMap).map(k => ({
            fornecedor: k,
            valor: fornecedorMap[k]
        })).sort((a, b) => b.valor - a.valor).slice(0, 5);

        const gastosMensais = Object.keys(mensalMap).map(k => ({
            mes: k,
            valor: mensalMap[k]
