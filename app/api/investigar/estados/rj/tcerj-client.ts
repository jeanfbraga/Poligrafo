...
export async function buscarContratosTceRj(municipio: string, cnpjAlvos: string[]): Promise<any[]> {
    if (!municipio) return [];
    ...
    try {
        ...
        console.error(`[TCE-RJ] Erro ao buscar contratos para ${municipio}:`, error);
    }
    
    return despesas;
}

export async function buscarComprasDiretasTceRj(municipio: string, cnpjAlvos: string[]): Promise<any[]> {
    if (!municipio) return [];
    ...
    try {
        ...
        console.error(`[TCE-RJ] Erro ao buscar compras diretas para ${municipio}:`, error);
    }
