...
export async function buscarMunicipalRS(nomeBuscado: string): Promise<{
    ...
    console.log(`[>> MUNICIPAL RS ENTRY] buscarMunicipalRS chamado para: ${nomeBuscado}`);
...
export async function buscarCodigoMunicipioRS(nomeMunicipio: string): Promise<string | null> {
    const nomeLimpo = nomeMunicipio.toLowerCase().trim();

    if (!municipiosCacheRS) {
        try {
            const res = await fetchWithTimeout(`${API_BASE}/auxiliar/municipios.json`, { timeout: TIMEOUT_RS });
            if (!res.ok) throw new Error(`Status ${res.status}`);
            
            const raw = await res.json();
            const data = Array.isArray(raw) ? raw : (raw.municipios || Object.values(raw).find(v => Array.isArray(v)) || []);
            
            municipiosCacheRS = {};
            data.forEach((m: any) => {
                if (m.NOME_MUNICIPIO && m.COD_MUNICIPIO) {
                    municipiosCacheRS![m.NOME_MUNICIPIO.toLowerCase().trim()] = String(m.COD_MUNICIPIO);
                }
            });
        } catch (e) {
...
async function extractRsData(url: string, orgaoCodigo?: string): Promise<any> {
    try {
        ...
        if (orgaoCodigo) {
            return (items as any[]).find((i: any) => String(i.CD_Orgao).startsWith(orgaoCodigo));
        }
        return items;
    } catch (e) {
        return null;
...
export async function buscarDespesasMunicipalRS(identificador: string, nomeParaBusca?: string, municipioUri?: string, casa?: string): Promise<any[]> {
    if (!municipioUri) {
    ...
    for (const ano of anosBusca) {
        ...
        if (educacao) {
            formatados.push({
                ...
                nomeFornecedor: educacao.NM_Orgao || `Prefeitura Municipal de ${municipioNomeLimpo.toUpperCase()}`,
                cnpjCpfFornecedor: "", 
        ...
        if (saude) {
            formatados.push({
                ...
                nomeFornecedor: saude.NM_Orgao || `Prefeitura Municipal de ${municipioNomeLimpo.toUpperCase()}`,
                cnpjCpfFornecedor: "", 
        ...
        if (gestao) {
            ...
            const percentualPessoal = rcl > 0 ? ((despPessoal / rcl) * 100).toFixed(2) : '0';
            
            formatados.push({
                tipoDespesa: "Gestão Fiscal LRF (TCE-RS)",
                nomeFornecedor: gestao.NM_Orgao || `Prefeitura Municipal de ${municipioNomeLimpo.toUpperCase()}`,
                cnpjCpfFornecedor: "",
