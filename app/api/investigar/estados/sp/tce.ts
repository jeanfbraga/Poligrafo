...
export async function buscarDespesasVereadorSP(identificador: string, municipioTce: string) {
    ...
    try {
        ...
        for (const { ano, mes } of consultas) {
            ...
            const formatado = notasDoPolitico
                .map((d: any) => {
                    let valorStr = (d.vl_despesa || '0').toString().replace(/\./g, '').replace(',', '.');
                    let valorNum = Number(valorStr);

                    return {
                        cnpjCpfFornecedor: d.id_fornecedor ? d.id_fornecedor.replace(/\D/g, '') : "",
                        nomeFornecedor: d.nm_fornecedor,
                        tipoDespesa: d.evento || d.funcao || "DESPESA MUNICIPAL",
                        valorDocumento: isNaN(valorNum) ? 0 : valorNum,
                        dataDocumento: `${ano}-${String(mes).padStart(2, '0')}-01`,
                        urlDocumento: null
                    };
                })
                .filter((d: any) => d.cnpjCpfFornecedor.length >= 11 && d.nomeFornecedor);
        ...
        console.error(`[MUNICIPAL SP] Erro ao buscar despesas de ${municipioTce} no TCE-SP:`, error);
        return [];
    }
}
