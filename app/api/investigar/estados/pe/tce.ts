...
export async function buscarMunicipalPE(nomeBuscado: string): Promise<{
    ...
    console.log(`[>> MUNICIPAL PE ENTRY] buscarMunicipalPE chamado para: ${nomeBuscado}`);
...
export async function buscarDespesasMunicipalPE(identificador: string, nomeParaBusca?: string, municipioUri?: string, casa?: string): Promise<any[]> {
    ...
    try {
        ...
        if (casa === 'PREFEITURA' && municipioUri) {
            ...
            console.log(`[TCE-PE] Alvo é PREFEITO. Buscando Despesas Municipais e Contratos para: ${municipioAjustado}...`);
            ...
            const res = await fetchWithTimeout(urlContratos, { timeout: PARAMS_TIMEOUT });
            if (res.ok) {
                ...
                if (payload?.resposta?.status === "OK" && payload?.resposta?.conteudo) {
                    ...
                    contratos.slice(0, 30).forEach((c: any) => {
                        despesasApuradas.push({
                            ...
                            nomeFornecedor: c.FORNECEDOR || nomeParaBusca || "Não Identificado",
                            tipoDespesa: `Contrato: ${c.OBJETO || 'N/I'} (UG: ${c.NOMEUNIDADEGESTORA})`,
                            valorDocumento: Number(c.VALORCONTRATO || 0),
                            dataDocumento: `${c.ANOREFERENCIA}-01-01`,
                            urlDocumento: `https://sistemas.tce.pe.gov.br/` 
        ...
        console.warn(`[TCE-PE] Falha ao varrer Extrator PE para o alvo ${municipioUri || docLimpo}.`, e);
    }
