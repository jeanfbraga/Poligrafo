...
export async function buscarDespesasMunicipalPB(identificador: string, nomeParaBusca?: string, municipioUri?: string, casa?: string) {
    if (!municipioUri) {
        console.log(`[TCE-PB] Sem município definido. Redirecionando para Proxy OSINT genérico.`);
        const proxy = await buscarProxyOsint(identificador, nomeParaBusca);
        return proxy.despesasFederais || [];
    }

    console.log(`[TCE-PB] Iniciando extração nativa de Despesas/Contratos para: ${identificador} em ${municipioUri} (${casa})`);
    ...
    try {
        ...
        const res = await fetchWithTimeout(urlDespesas, { headers, timeout: TIMEOUT_PB });
                ...
                data.forEach((d: any, index: number) => {
                    malhaTce.push({
                        ...
                        data: {
                            ...
                            fornecedor: d.credor || d.favorecido || nomeParaBusca || 'Desconhecido',
                            data: d.data_emissao || d.data || 'N/I',
                            url: d.url || urlDespesas,
                            descricao: d.historico || d.objeto || 'Despesa registrada no TCE-PB (Sagres)'
                        }
                    });
                });
            }
        } else if (res.status === 403 || res.status === 503) {
            console.warn(`[TCE-PB] Bloqueio WAF (Cloudflare Turnstile) detectado no endpoint de despesas. Status: ${res.status}`);
        }
    } catch (e: any) {
    ...
    try {
        ...
        const resContratos = await fetchWithTimeout(urlContratos, { headers, timeout: TIMEOUT_PB });
                ...
                dataC.forEach((c: any, index: number) => {
                    malhaTce.push({
                        ...
                        data: {
                            ...
                            fornecedor: c.contratado || c.favorecido || nomeParaBusca || 'Desconhecido',
                            data: c.data_assinatura || c.data || 'N/I',
                            url: c.url || urlContratos,
                            descricao: c.objeto || 'Contrato firmado na esfera municipal (PB)'
                        }
                    });
                });
            }
    ...
    if (malhaTce.length === 0) {
        console.log(`[TCE-PB] Sem retornos do Sagres. Acionando Fallback do TransfereGov/Federal.`);
