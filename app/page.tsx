...
import React, { useMemo, useState, KeyboardEvent, useCallback, useEffect, useRef } from "react";
import congressoIndex from "@/lib/data/congresso-index.json";
...
import { Terminal, Search, Loader2, BrainCircuit, Fingerprint, X, Download, Map as MapIcon, MapPin, Share2, ChevronDown, ShieldAlert, FileText, Landmark, AlertTriangle, User, DollarSign, Briefcase, Building2, Users } from "lucide-react";
import { Drawer } from 'vaul';
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { ShareDialog, ShareData } from "@/components/ShareDialog";
...
function DashboardArea() {
  ...
  const onConnect = useCallback(
    (params: Connection) => setEdges((eds) => addEdge({ ...params, animated: true, style: { stroke: '#4ade80', strokeWidth: 2 } }, eds)),
    [setEdges],
  );
  ...
  const [apiWarnings, setApiWarnings] = useState<{fonte: string; mensagem: string}[]>([]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("poligrafo:search", handlePoligrafoSearch);
  }, []);
  const [beneficiaryContracts, setBeneficiaryContracts] = useState<any[]>([]);
  const [loadingBeneficiaryContracts, setLoadingBeneficiaryContracts] = useState<boolean>(false);
  ...
  const handleOpenDashboard = React.useCallback(async (nomeVereador: string) => {
    setDashboardNome(nomeVereador);
    setDashboardOpen(true);
    setDashboardLoading(true);
    setDashboardData(null);
    try {
      const res = await fetch(`/api/investigar/estados/rj/dashboard-cota?nome=${encodeURIComponent(nomeVereador)}`);
      if (res.ok) {
  ...
  const [statusMessage, setStatusMessage] = useState<string>("Insira o nome de um político para começar a investigar.");
  ...
  React.useEffect(() => {
    const interval = setInterval(() => {
      if (nodesToAddBuffer.current.length > 0) {
        ...
        setNodes(prev => {
          const prevMap: Map<string, Node> = new Map(prev.map(n => [n.id, n as any]));
          nodesSnapshot.forEach((n: Node) => {
            if (prevMap.has(n.id)) {
              // Update existing node with fresh data
              const existing = prevMap.get(n.id)!;
              prevMap.set(n.id, { ...existing, data: { ...existing.data, ...n.data } });
            } else {
              // Add new node
              prevMap.set(n.id, n);
            }
  ...
  const [candidatosHomonimos, setCandidatosHomonimos] = useState<any[] | null>(null);

  const handleShareClick = useCallback((nodeData: any, type: string) => {
    const pessoaNode = getNodes().find((n: Node) => n.type === 'PESSOA');
    const getValor = () => {
      if (nodeData.valor !== undefined) return Number(nodeData.valor);
      if (nodeData._empenhado !== undefined) return Number(nodeData._empenhado);
      return undefined;
    };
    setShareData({
      politicoNome: String((pessoaNode?.data as any)?.label || "Desconhecido"),
      ...
      achadoTitulo: nodeData.label || nodeData.objeto || "Registro Encontrado",
      ...
      achadoFonteUrl: nodeData.urlDocumento || nodeData.url_documento || nodeData.link_documento || undefined
    });
    setIsShareOpen(true);
  }, [getNodes]);

  const handleToggleEmendas = useCallback((hubId: string) => {
    setNodes((nds) => {
  ...
  const nodeTypes = useMemo(() => ({
    PESSOA: PessoaNode,
    DESPESA: (props: any) => <DespesaNode {...props} data={{...props.data, onShare: handleShareClick}} />,
    CONTRATO: (props: any) => <ContratoNode {...props} data={{...props.data, onShare: handleShareClick}} />,
    EMENDA: (props: any) => <EmendaNode {...props} data={{...props.data, onShare: handleShareClick}} />,
    EMENDA_RESUMO: (props: any) => <EmendaResumoNode {...props} data={{...props.data, onOpenDashboard: handleOpenDashboard}} />,
    EMPRESA: (props: any) => <EmpresaNode {...props} data={{...props.data, onShare: handleShareClick}} />,
    SOCIO: SocioNode,
    ORGAO: OrgaoNode,
    PROCESSO_JUDICIAL: (props: any) => <ProcessoJudicialNode {...props} data={{...props.data, onShare: handleShareClick}} />,
    RESUMO_GASTOS: (props: any) => <RaioXGastosNode {...props} data={{...props.data, onOpenDashboard: handleOpenDashboard}} />,
  }), [handleShareClick, handleToggleEmendas, handleOpenDashboard]);
  ...
  const handlePivotCNPJ = async (cnpj: string, origemId: string) => {
    ...
    setNodes((nds) => nds.map((n) => n.id === origemId ? { ...n, data: { ...n.data, isSearching: true } } : n));

    try {
      const response = await fetch(`/api/investigar/cnpj?cnpj=${cnpj}&origemId=${origemId}`, { signal: controller.signal });
            ...
            try {
              ...
              if (event.tipo === 'STATUS') {
                ...
                setNodes((nds) => nds.map((n) =>
                  n.data?.isSearching ? { ...n, data: { ...n.data, currentStatus: event.payload.msg } } : n
                ));
                ...
                let multiplier = 0;
                if (siblingsSoFar > 0) {
                  multiplier = (siblingsSoFar % 2 !== 0 ? -1 : 1) * Math.ceil(siblingsSoFar / 2);
                }

                let px = baseX + (multiplier * spacingX);
                let py = baseY + spacingY;

                if (nodeParams.type === 'CONTRATO') { // Joga os contratos pro lado
                ...
                if (nodeParams._origemId) {
                  let edgeColor = nodeParams.type === 'SOCIO' ? '#a855f7' : '#3b82f6';
                  let edgeLabel = nodeParams.type === 'SOCIO' ? 'SÓCIO (QSA)' : 'FORNECEDOR';
                  let isAnimated = true;
                  // VERIFICAÇÃO DE NEPOTISMO CRUZADO
                  if (nodeParams.type === 'SOCIO') {
                    const pessoaNode = nodesToAddBuffer.current.find((n: Node) => n.type === 'PESSOA') || nodes.find((n: Node) => n.type === 'PESSOA');
                    if (pessoaNode && pessoaNode.data?.nomeCivil) {
                      const nomePoliticoArr = String(pessoaNode.data.nomeCivil).trim().split(' ');
                      const sobrenomePolitico = nomePoliticoArr[nomePoliticoArr.length - 1].toLowerCase();
                      const nomeSocioArr = String(nodeParams.data.label).trim().split(' ');
                      const sobrenomeSocio = nomeSocioArr[nomeSocioArr.length - 1].toLowerCase();
                      const sobrenomesComuns = ['silva', 'santos', 'oliveira', 'souza', 'pereira', 'costa', 'carvalho', 'almeida', 'ferreira', 'ribeiro'];
                      if (sobrenomePolitico === sobrenomeSocio && !sobrenomesComuns.includes(sobrenomePolitico) && sobrenomePolitico.length > 3) {
                  ...
                  edgesToAddBuffer.current.push({
                    ...
                    label: edgeLabel,
                    animated: isAnimated,
                    style: { stroke: edgeColor, strokeWidth: edgeColor === '#ef4444' ? 3 : 2 },
                  });
                }
              }
            } catch (e) { }
      ...
      if (err.name === 'AbortError') {
         ...
         setIsLoading(false);
         setNodes((nds) => nds.map((n) => n.id === origemId ? { ...n, data: { ...n.data, isSearching: false, currentStatus: undefined } } : n));
      ...
      setNodes((nds) => nds.map((n) => n.id === origemId ? { ...n, data: { ...n.data, isSearching: false } } : n));
    }
  };

  const handleOrgaoSearch = async (orgaoNodeId: string, nomePolitico: string, casaLegislativa: string) => {
    if (abortController) abortController.abort();
    const controller = new AbortController();
    setAbortController(controller);

    setIsLoading(true);
    setStatusMessage(`Aprofundando buscas no ${casaLegislativa} para o parlamentar ${nomePolitico}...`);

    setNodes((nds) => nds.map((n) => n.id === orgaoNodeId ? { ...n, data: { ...n.data, isSearching: true } } : n));

    try {
      // Rota específica baseada na casa
      const apiPath = casaLegislativa === 'ALERJ' ? `/api/investigar/alerj/despesas` : '';
      if (!apiPath) throw new Error("Integração Deep Dive indisponível para este órgão no momento.");

      const response = await fetch(`${apiPath}?nome=${encodeURIComponent(nomePolitico)}&origemId=${orgaoNodeId}`, { signal: controller.signal });
          ...
          if (line.startsWith('data: ')) {
            ...
            try {
              ...
              if (event.tipo === 'STATUS') {
                ...
                setIsLoading(false);
                setTimeout(() => {
                  toast.success(`> [DRILlDOWN COMPLETO] Verificações no órgão de ${nomePolitico} processadas.`);
                  setNodes((nds) => nds.map((n) => n.id === orgaoNodeId ? { ...n, data: { ...n.data, isSearching: false, hasDeepDive: false } } : n));
                }, 1500);
              } else if (event.tipo === 'NODE_NOVO') {
                const nodeParams = event.payload;
                ...
                if (event.payload.despesas && event.payload.despesas.length > 0) {
                  setEvidencias(prev => {
                    const deduplicated = event.payload.despesas.filter((nova:any) => 
                      !prev.some(evt => evt.cnpjCpfFornecedor === nova.cnpjCpfFornecedor && evt.valorDocumento === nova.valorDocumento)
                    );
      ...
      if (err.name === 'AbortError') {
         ...
         setIsLoading(false);
         setNodes((nds) => nds.map((n) => n.id === orgaoNodeId ? { ...n, data: { ...n.data, isSearching: false, currentStatus: undefined } } : n));
         return;
      }
      setErrorMsg(err.message);
      toast.error(`> [ERRO DE DRILLDOWN] ${err.message}`);
      ...
      setNodes((nds) => nds.map((n) => n.id === orgaoNodeId ? { ...n, data: { ...n.data, isSearching: false } } : n));
    }
  };
  ...
  const handleSocioSearch = async (nomeSocio: string, origemId: string) => {
    ...
    try {
      const response = await fetch(`/api/investigar/socio?nome=${encodeURIComponent(nomeSocio)}&origemId=${origemId}`, { signal: controller.signal });
      if (!response.body) throw new Error("Falha no stream.");
      const reader = response.body.getReader();
      const decoder = new TextDecoder();
      let buffer = '';
          ...
          if (line.startsWith('data: ')) {
            ...
            try {
              ...
              if (event.tipo === 'STATUS') {
                ...
                setIsLoading(false);
                setTimeout(() => {
                  toast.success(`> [BUSCA REVERSA CONCLUÍDA] Conexões de ${nomeSocio} mapeadas.`);
                  setNodes((nds) => nds.map((n) => n.id === origemId ? { ...n, data: { ...n.data, isSearching: false, currentStatus: undefined } } : n));
                }, 1500);
              } else if (event.tipo === 'NODE_NOVO') {
                const nodeParams = event.payload;
  ...
  const handleInvestigarContratos = async (cnpj: string, origemId: string) => {
    ...
    setIsLoading(true);
    setStatusMessage(`Buscando histórico de licitações no PNCP (Portal Nacional de Contratações Públicas)...`);
    ...
    try {
      ...
      const response = await fetch(`/api/investigar/licitacoes?cnpj=${cnpj}&politico=${encodeURIComponent(politicoNome)}`, { signal: controller.signal });
      const data = await response.json();

      if (data.hasContracts && data.contracts?.length > 0) {
        setStatusMessage(`> ${data.contracts.length} contratos localizados. Renderizando fluxo financeiro...`);
        
        const localPositions = new Map<string, { x: number, y: number }>();
        nodes.forEach(n => localPositions.set(n.id, n.position));
        const parentPos = localPositions.get(origemId) || { x: window.innerWidth / 2, y: 150 };

        data.contracts.forEach((c: any, index: number) => {
          const cid = `pncp-${c.numeroControlePNCP}`;
          const aiAnalysis = data.aiAnalysis?.contratos_avaliados?.find((a:any)=>a.numeroControlePNCP === c.numeroControlePNCP);
          ...
          const nodeParams = {
            ...
            data: {
              ...
              enquadramento_normativo: aiAnalysis?.enquadramento_normativo || "-",
              motivo_ia: aiAnalysis?.motivo_ia || (data.aiAnalysis?.score_letalidade_geral > 50 ? "Risco sistêmico identificado no lote." : null),
              score_letalidade: aiAnalysis?.score_letalidade ?? (data.aiAnalysis?.score_letalidade_geral || 20)
            }
          };
        ...
        toast.success(`> [PNCP] ${data.contracts.length} contratos integrados.`);
      } else {
        toast.info(`> [PNCP] Nenhum contrato relevante encontrado nos últimos 8 anos.`);
      }
      setIsLoading(false);
    } catch (err: any) {
      ...
      setNodes((nds) => nds.map((n) => n.id === origemId ? { ...n, data: { ...n.data, isSearching: false, currentStatus: null } } : n));
    }
  };

  const handleSearch = async (refOverride?: string | any, nomeOverride?: string | any) => {
    ...
    if (!refOverride && !selectedUf) {
      setStatusMessage('> SELECIONE A ALÇADA (ESTADO) DO POLÍTICO ANTES DE BUSCAR.');
      return;
    }
    ...
    setNodes([{
      ...
      data: { label: termo.toUpperCase(), isSearching: true, currentStatus: 'Iniciando conexão...' },
    ...
    setExpandedNodes({});
    setStatusMessage('Estabelecendo conexão segura com bases governamentais...');
    ...
    try {
            ...
            try {
              ...
              if (event.tipo === 'STATUS') {
                ...
                setApiWarnings(prev => {
                  ...
                  if (prev.some(w => w.fonte === event.payload.fonte)) return prev;
                  return [...prev, { fonte: event.payload.fonte, mensagem: event.payload.mensagem }];
                });
              }
              else if (event.tipo === 'CANDIDATOS_ENCONTRADOS') {
                const candidatos = event.payload.candidatos;
                if (candidatos.length === 1) {
                  setStatusMessage(`> 1 candidato localizado. Iniciando investigação automática...`);
                  setSearchTerm(candidatos[0].nome);
                  setTimeout(() => handleSearch(candidatos[0].ref, candidatos[0].nome), 100);
                  break;
                } else {
                  setCandidatosHomonimos(candidatos);
                  setIsLoading(false);
                  setStatusMessage(`> ${candidatos.length} homônimos detectados. Selecione o alvo correto.`);
                }
              }
              else if (event.tipo === 'DONE') {
                setStatusMessage(event.payload?.msg ? `> ${event.payload.msg}` : '> [PROCESSO FINALIZADO] Evidências extraídas para a Sandbox.');
                setIsLoading(false);

                // Mantém a animação holográfica visível por pelo menos 1.5s pra não parecer um bug de carregamento instantâneo
                setTimeout(() => {
                  toast.success("> [PROCESSO FINALIZADO] Dossiê completo gerado com sucesso.");
                  setNodes((nds) => nds.map((n) => n.type === 'PESSOA' ? { ...n, data: { ...n.data, isSearching: false, currentStatus: undefined } } : n));
                ...
                const STRUCTURAL_TYPES = ['PESSOA', 'EMPRESA', 'ORGAO', 'EMENDA_RESUMO', 'EMENDA', 'PROCESSO_JUDICIAL', 'CONTRATO', 'RESUMO_GASTOS'];
                if (STRUCTURAL_TYPES.includes(nodeParams.type)) {
                  ...
                  if (nodeParams.type === 'PESSOA') {
                    ...
                    setNodes(nds => nds.map(n => {
                      if (n.id === 'loading-pessoa') {
                        return {
                          ...
                          data: { ...n.data, ...nodeParams.data, isSearching: true },
                          position: { x: (window.innerWidth - 320) / 2 - 144, y: 150 },
                          className: "animate-[customFadeIn_0.5s_ease-out]"
                        };
                      }
                    ...
                    nodesToAddBuffer.current.push({...nodeParams, data: { ...nodeParams.data, isSearching: true }});
                  ...
                  } else if (nodeParams.type === 'EMENDA' && emendaHubIdRef.current) {
                    // Emendas individuais: layout radial no mobile, coluna vertical oculta no desktop
                    const hubPos = nodesToAddBuffer.current.find((n: Node) => n.id === emendaHubIdRef.current)?.position 
                      || { x: (window.innerWidth - 320) / 2 + 356, y: 350 };
                    ...
                    const px = (window.innerWidth - 320) / 2 - 300 + (Math.random() * 600);
                  ...
                  if (pessoaId && nodeParams.type !== 'PESSOA') {
                    ...
                    edgesToAddBuffer.current.push({
                      ...
                      target: nodeParams.id,
                      animated: true,
                      style: { stroke: edgeColorMap[nodeParams.type] || '#22c55e', strokeWidth: 1.5 }
                    });
                  }
                  ...
                  const px = (window.innerWidth - 320) / 2 - 144 + (letalNodesCount % 3 === 0 ? 0 : letalNodesCount % 3 === 1 ? -350 : 350);
                  ...
                  if (pessoaId) {
                    edgesToAddBuffer.current.push({
                      ...
                      animated: true,
                      style: { stroke: isRedFlag ? '#ef4444' : '#ca8a04', strokeWidth: isRedFlag ? 3 : 2 }
                    });
                  }
                } else {
                  // DESPESAS COM SCORE BAIXO (< 60): Joga silenciosamente na Sidebar
                  setEvidencias((prev: any) => {
                    const index = prev.findIndex((e: any) => e.id === nodeParams.id);
                    if (index > -1) {
                      const newPrev = [...prev];
                      newPrev[index] = { ...newPrev[index], data: { ...newPrev[index].data, ...nodeParams.data } };
                      return newPrev;
                    }
                ...
                setNodes((nds) => nds.map((n) => {
                  if (n.type === 'PESSOA') {
                    return {
                      ...
                      data: {
                        ...n.data,
                        alertas: [...(Array.isArray(n.data.alertas) ? n.data.alertas : []), event.payload.msg]
                      }
                    };
                  }
                  return n;
      ...
      if (err.name === 'AbortError') {
         setStatusMessage('> [OPERAÇÃO ABORTADA PELO USUÃRIO]. Nós preservados.');
         toast.warning('> Busca principal cancelada pelo operador.');
         setIsLoading(false);
         // Desliga loading globalmente em todos os nós
         setNodes((nds) => nds.map((n) => n.data?.isSearching ? { ...n, data: { ...n.data, isSearching: false, currentStatus: undefined } } : n));
         return;
      }
      setErrorMsg(err.message);
  ...
  const handleExportDossie = async () => {
    ...
    try {
      ...
      const entidadesRisco = [...nodes.filter(n => (Number(n.data?.score_letalidade) || 0) >= 60 || n.type === 'CONTRATO'), ...evidencias.filter((e: any) => (Number(e.data?.score_letalidade) || 0) >= 60 || e.type === 'CONTRATO')]
        .map(n => ({...n.data, type: n.type}))
        .sort((a: any, b: any) => (b?.score_letalidade || 0) - (a?.score_letalidade || 0));
      ...
      const urlsEvi = entidadesRisco
        .flatMap((d: any) => {
          ...
          if (d?.numeroControlePNCP) links.push(`https://pncp.gov.br/app/contratos?q=${encodeURIComponent(d.numeroControlePNCP)}`);
          return links;
        })
        .filter((u: string) => u && String(u).startsWith('http'));

      const pessoaNode = nodes.find(n => n.type === 'PESSOA');
      const nomePolitico = pessoaNode?.data?.label || searchTerm || 'Desconhecido';

      // 3. Chama a API
      const res = await fetch('/api/exportar-dossie', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nomePolitico, despesasCriticas: entidadesRisco, urlsNotasFiscais: urlsEvi })
      });
  ...
  const onDrop = useCallback(
      ...
      try {
        ...
        if (pessoaRaiz) {
          ...
          if (newNode.type === 'DESPESA') {
            strokeColor = score >= 85 ? '#ef4444' : (score >= 60 ? '#eab308' : '#64748b');
          ...
          const newEdge: Edge = {
            ...
            target: newNode.id,
            label: newNode.type === 'DESPESA' ? `EVIDÊNCIA (${score}/100)` : "AUTOR_DE",
            animated: true,
    ...
    [screenToFlowPosition, nodes, setNodes, setEdges, setEvidencias],
  );
  ...
  const getSidebarClasses = (nodeData: any) => {
    if (nodeData.type === 'DESPESA') {
      ...
      if (score >= 85) return {
        cardBody: 'border-red-500 text-red-500 bg-red-950/20 shadow-[0_0_10px_rgba(239,68,68,0.3)]',
        iconCode: <ShieldAlert className="w-3 h-3 text-red-500 shrink-0" />,
        scoreBadge: <Badge className="bg-red-900 absolute top-1 right-2 text-xs rounded-none">ðŸ”¥ {score}</Badge>
      };
      if (score >= 60) return {
        cardBody: 'border-yellow-600 text-yellow-500 bg-yellow-950/20',
        iconCode: null,
        scoreBadge: <Badge className="bg-yellow-900 text-yellow-500 absolute top-1 right-2 text-xs rounded-none border border-yellow-700">ï¸ {score}</Badge>
      };
      return {
        cardBody: 'border-slate-700 text-slate-400 bg-slate-900/10',
        iconCode: null,
        scoreBadge: <Badge className="bg-slate-800 text-slate-400 absolute top-1 right-2 text-xs rounded-none border border-slate-700">{score}</Badge>
      };
    }
    if (nodeData.type === 'CONTRATO') return {
    ...
    if (nodeData.type === 'EMENDA' || nodeData.type === 'EMENDA_RESUMO') {
      const isFantasma = nodeData.data?._isFantasma ?? nodeData.data?.isFantasma;
      const risco = nodeData.data?._riscoTipo?.nivel;
      if (isFantasma || risco === 'CRÍTICO') return {
        cardBody: 'border-red-500 text-red-500 bg-red-950/20 shadow-[0_0_10px_rgba(239,68,68,0.3)]',
        iconCode: <Landmark className="w-3 h-3 text-teal-500 shrink-0" />,
        scoreBadge: <Badge className="bg-red-900 absolute top-1 right-2 text-xs rounded-none">{isFantasma ? 'FANTASMA' : 'PIX/RP9'}</Badge>
      };
      return {
        cardBody: 'border-teal-500 text-teal-500 bg-teal-950/10',
        iconCode: <Landmark className="w-3 h-3 text-teal-500 shrink-0" />,
        scoreBadge: <Badge className="bg-teal-900 text-teal-400 absolute top-1 right-2 text-xs rounded-none border border-teal-700">{nodeData.data?._percentualExecucao ?? 0}%</Badge>
      };
    }
  ...
  const clearAll = () => { setNodes([]); setEdges([]); setEvidencias([]); setErrorMsg(''); setApiWarnings([]); setSearchTerm(''); setSelectedUf(''); setStatusMessage('Insira o nome de um político para começar a investigar.'); };
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-black text-green-500 font-mono overflow-hidden">
      ...
      <header className={`h-auto md:h-14 py-2 md:py-0 border-b border-green-500 bg-black flex-col md:flex-row items-center justify-between px-4 md:px-6 shrink-0 z-50 relative gap-2 md:gap-0 ${isMobile ? 'hidden' : 'flex'}`}>
        <div className="flex items-center justify-between w-full md:w-auto">
          <div className="flex items-center gap-3">
            <button onClick={clearAll} className="flex items-center gap-3 cursor-pointer hover:opacity-80 transition-opacity outline-none">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        {nodes.length > 0 && !isMobile && (
          <aside className="w-80 bg-[#0a0f0a] border-r border-green-500/50 shrink-0 hidden md:flex flex-col pt-16 z-20 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
            <div className="px-4 py-3 border-b border-green-900 flex justify-between items-center bg-black sticky top-0 shadow-md">
              ...
              <Badge variant="outline" className="bg-black text-green-600 border-green-900 rounded-none text-xs">
              ...
              {evidenciasOrdenadas.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="text-xs font-bold truncate tracking-wide pr-8" title={item.data.label}>
                      {item.data.label || 'Unknown'}
                    </h3>
                    {(item.data.valor || item.data.valor === 0) && (
                      <div className="mt-1">
                        <p className="text-sm font-bold opacity-90">
                          R$ {Number(item.data.valor).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </p>
                        <p className="text-xs mt-1 opacity-60 uppercase font-mono tracking-wider">
                          {item.data.dataDocumento ? new Date(item.data.dataDocumento).toLocaleDateString('pt-BR', { timeZone: 'UTC' }) : 'Sem Data'}
                        </p>
                      </div>
                    )}
                    {item.data.motivo_ia && (
        ...
        <main className="flex-1 relative w-full h-full bg-[#050505]">
          ...
          {nodes.length === 0 && !isLoading && !errorMsg && (!candidatosHomonimos || candidatosHomonimos.length === 0) && (
            <HomeDashboard />
          )}
          ...
          {errorMsg && !isLoading && (
            <div className="absolute inset-0 flex items-center justify-center z-50 bg-black backdrop-blur-[2px] pointer-events-none">
              <div className="border border-red-500/50 bg-black p-6 max-w-md text-center pointer-events-auto shadow-[0_0_40px_rgba(239,68,68,0.25)]">
                ...
                <p className="text-sm text-red-500 mb-2 uppercase tracking-widest font-bold">FALHA DE EXTRAÇÃO (OSINT)</p>
                <p className="text-xs text-red-500/80 leading-relaxed font-mono">&gt; {errorMsg}</p>
                <button
                  onClick={() => setErrorMsg('')}
          ...
          {apiWarnings.length > 0 && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-40 flex flex-col gap-2 w-full max-w-lg px-4">
              {apiWarnings.map((w, i) => (
                <div key={w.fonte} className="border border-yellow-500/60 bg-black backdrop-blur-sm p-4 shadow-[0_0_20px_rgba(234,179,8,0.15)] animate-[customFadeIn_0.3s_ease-out]">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button
                      onClick={() => setApiWarnings(prev => prev.filter((_, idx) => idx !== i))}
            ...
            <div className="absolute inset-0 flex items-center justify-center z-30 bg-black backdrop-blur-sm">
              <div className="border border-green-500 bg-black p-6 max-w-lg w-full mx-4 shadow-[0_0_30px_rgba(34,197,94,0.3)]">
                <div className="flex items-center gap-2 mb-4 pb-3 border-b border-green-500">
                  ...
                  <h2 className="text-sm font-bold tracking-widest uppercase text-yellow-500">HOMÔNIMOS DETECTADOS</h2>
                </div>
                <p className="text-xs text-green-600 mb-4">
                  &gt; A busca retornou {candidatosHomonimos.length} perfis. Selecione o alvo correto para iniciar a investigação:
                </p>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-bold text-green-400 uppercase tracking-wider">{c.nome}</span>
                        </div>
                        <Badge variant="outline" className={`rounded-none text-xs uppercase ${c.casa === 'CAMARA' ? 'border-green-600 text-green-500' :
                          c.casa === 'SENADO' ? 'border-blue-600 text-blue-400' :
          ...
          >
            ...
            {mobileActive ? (
              ...
              <ReactFlow
                ...
                onNodeClick={(_, node) => {
                  ...
                  if (['PESSOA', 'DESPESA', 'CONTRATO', 'EMENDA', 'EMENDA_RESUMO', 'EMPRESA', 'SOCIO'].includes(node.type as string)) {
                ...
                defaultEdgeOptions={{
                  style: { strokeWidth: 2 },
                  labelBgStyle: { fill: '#0f172a', fillOpacity: 1, stroke: '#334155', strokeWidth: 1, rx: 6, ry: 6 },
              ...
              <div className="absolute inset-0 w-full h-full bg-[#050505] flex items-center justify-center flex-col px-4" style={{ backgroundImage: 'radial-gradient(circle, #002200 1px, transparent 1px)', backgroundSize: '24px 24px' }}>
            ...
            {!isMobile && nodes.length > 0 && !isLoading && (
              ...
              >
                {isExporting ? (
                  <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> GERANDO DOSSIÊ...</>
                ) : (
      ...
      <Sheet open={!!selectedNode} onOpenChange={(open: boolean) => !open && setSelectedNode(null)}>
        <SheetContent className="bg-black border-l-2 border-green-500 font-mono text-green-500 overflow-y-auto w-[400px] sm:w-[540px] pt-12 pr-6">
          {selectedNode && selectedNode.type === "PESSOA" && (
            <>
              <SheetHeader className="pb-4 mb-4 border-b border-green-900 border-dashed relative">
                <VisuallyHidden>
                  <SheetTitle>Detalhes do Alvo {selectedNode.data.label}</SheetTitle>
                  <SheetDescription>Informações detalhadas sobre o político selecionado.</SheetDescription>
                </VisuallyHidden>
                <Badge variant="outline" className="w-fit mb-2 text-xs uppercase rounded-none border bg-green-900/30 text-green-500 border-green-500 pr-4">
                  {selectedNode.data.cargo} • {selectedNode.data.uf}
                </Badge>
                <div className="text-lg font-bold uppercase tracking-wider text-green-500">
                  {selectedNode.data.label}
                </div>
                {selectedNode.data.cpf && selectedNode.data.cpf !== "000.000.000-00" && selectedNode.data.cpf !== "00000000000" ? (
                ...
                {selectedNode.data.patrimonio !== undefined && (
                  <div>
                    <h3 className="text-xs uppercase font-bold text-yellow-600 mb-2 border-b border-yellow-900/50 pb-1 flex items-center gap-2">
                      <DollarSign className="w-4 h-4" /> PATRIMÔNIO DECLARADO (TSE)
                    </h3>
                    <div className="p-3 border bg-yellow-950/10 border-yellow-900/30 text-yellow-500 text-center">
                      {selectedNode.data.patrimonio > 0 ? (
                        <p className="text-2xl font-bold tracking-widest">
                          R$ {selectedNode.data.patrimonio.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </p>
                      ) : (
                ...
                {selectedNode.data.alertasPessoais !== undefined && (
                  <div>
                    <h3 className="text-xs uppercase font-bold text-red-600 mb-2 border-b border-red-900/50 pb-1 flex items-center gap-2">
                      <ShieldAlert className="w-4 h-4" /> CADASTRO DE INIDÔNEOS (CGU/TSE)
                    </h3>
                    <div className={`p-3 border ${selectedNode.data.alertasPessoais.length > 0 ? 'bg-red-950/10 border-red-900/30 text-red-400' : 'bg-slate-900/10 border-slate-800 text-slate-500'}`}>
                      {selectedNode.data.alertasPessoais.length > 0 ? (
                        <ul className="space-y-3">
                          {selectedNode.data.alertasPessoais.map((alerta: string, index: number) => (
                            <li key={index} className="flex gap-2 text-xs break-words w-full">
                              <ShieldAlert className="h-4 w-4 shrink-0 mt-0.5 text-red-500 animate-pulse" />
                              <span className="leading-tight uppercase tracking-wide">{alerta}</span>
                            </li>
                          ))}
                        </ul>
                      ) : (
                        <p className="text-xs uppercase tracking-wider text-center">Nenhum Registro de Inidoneidade na Consulta Rápida</p>
                      )}
                    </div>
                  </div>
                )}
          ...
          {selectedNode && selectedNode.type === "DESPESA" && (
            <>
              <SheetHeader className={`pb-4 mb-4 border-b border-dashed relative ${selectedNode.data.score_letalidade >= 85 ? 'border-red-900' : selectedNode.data.score_letalidade >= 60 ? 'border-yellow-900' : 'border-slate-800'}`}>
                <VisuallyHidden>
                  <SheetTitle>Detalhes da Despesa</SheetTitle>
                  <SheetDescription>Informações detalhadas sobre o gasto parlamentar.</SheetDescription>
                </VisuallyHidden>
                <Badge variant="outline" className={`w-fit mb-2 text-xs uppercase rounded-none border pr-4 ${selectedNode.data.score_letalidade >= 85 ? 'bg-red-900/30 text-red-500 border-red-500' : selectedNode.data.score_letalidade >= 60 ? 'bg-yellow-900/30 text-yellow-500 border-yellow-500' : 'bg-slate-900/50 text-slate-400 border-slate-700'}`}>
                  {selectedNode.data.tipo || "Despesa CEAP"}
                </Badge>
                <div className={`text-lg font-bold pr-6 uppercase tracking-wider ${selectedNode.data.score_letalidade >= 85 ? 'text-red-500' : selectedNode.data.score_letalidade >= 60 ? 'text-yellow-500' : 'text-slate-400'}`}>
                  R$ {Number(selectedNode.data.valor).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                </div>
                <div className={`text-xs mt-1 mb-1 ${selectedNode.data.score_letalidade >= 85 ? 'text-red-400/70' : selectedNode.data.score_letalidade >= 60 ? 'text-yellow-400/70' : 'text-slate-500'}`}>
                  Fornecedor: {selectedNode.data.label}
                </div>
                <div className={`text-xs font-mono ${selectedNode.data.score_letalidade >= 85 ? 'text-red-400/70' : selectedNode.data.score_letalidade >= 60 ? 'text-yellow-400/70' : 'text-slate-500'}`}>
                  CNPJ: {selectedNode.data.documento}
                </div>
                {selectedNode.data.dataDocumento && (
                  <div className={`text-xs font-mono mt-1 ${selectedNode.data.score_letalidade >= 85 ? 'text-red-400/70' : selectedNode.data.score_letalidade >= 60 ? 'text-yellow-400/70' : 'text-slate-500'}`}>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  {selectedNode.data.urlDocumento ? (
                    ...
                    <div className="space-y-2 mt-2">
                      {(() => {
                        const pessoaNode = nodes.find((n: any) => n.type === 'PESSOA');
                        const fallback = getPortalTransparenciaFallback(pessoaNode?.data?.casa as string | undefined, pessoaNode?.data?.uri as string | undefined);
                ...
                {selectedNode.data.motivo_ia && (
                  <div className="mb-6">
                    <AIProgressBar score={selectedNode.data.score_letalidade} motivo={selectedNode.data.motivo_ia} />
                  </div>
                )}

                {/* O Risco do The Full OSINT vai aqui se existir */}
                {selectedNode.data.risco && selectedNode.data.risco.alertas && selectedNode.data.risco.alertas.length > 0 && (
                  <div>
                    <h3 className="text-xs uppercase font-bold text-red-600 mb-2 border-b border-red-900 pb-1">&gt; CRUZAMENTO DE DADOS OFICIAIS (CGU/TCU/RECEITA)</h3>
                    <ul className="space-y-2">
                      {selectedNode.data.risco.alertas.map((alerta: string, idx: number) => (
                        <li key={idx} className="flex gap-2 text-xs text-red-400 break-words w-full">
                ...
                {(selectedNode.data.documento || selectedNode.data.cnpjCpfFornecedor) && String(selectedNode.data.documento || selectedNode.data.cnpjCpfFornecedor).length > 11 && 
                 !(selectedNode.data.label?.toUpperCase().includes('ELEICAO') || 
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-800">
                    ...
                    <p className="text-xs text-slate-500 mb-3 leading-relaxed">
                      Verificar possível empresa de fachada via Google Street View pelo registro de CNPJ / Nome.
                    </p>
                    <a
                      href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
                        [selectedNode.data.label, selectedNode.data.municipio, selectedNode.data.uf, `CNPJ ${selectedNode.data.documento || selectedNode.data.cnpjCpfFornecedor}`, "Brasil"].filter(Boolean).join(" ")
                      )}`}
                      target="_blank"
                      rel="noopener noreferrer"
                ...
                {selectedNode.data.documento && selectedNode.data.documento.length > 11 && 
                 !(selectedNode.data.label?.toUpperCase().includes('ELEICAO') || 
                  selectedNode.data.label?.toUpperCase().includes('CAMPANHA') || 
                  selectedNode.data.cnae?.toUpperCase().includes('CAMPANHA')) && (
                  <div className="mt-8 pt-6 border-t border-slate-800">
                    ...
                    <p className="text-xs text-slate-500 mb-3 leading-relaxed">
                      Deseja extrair os vínculos empresariais, sócios (QSA) e o histórico de contratos públicos deste fornecedor?
                    </p>
                    {!expandedNodes[selectedNode.id] ? (
                      <Button
                        ...
                        onClick={() => {
                          ...
                          handlePivotCNPJ(selectedNode.data.documento, selectedNode.id);
                        }}
                      >
                        <Search className="mr-2 h-4 w-4" /> Aprofundar Investigação
                      </Button>
                    ) : (
          ...
          {selectedNode && (selectedNode.type === "CONTRATO" || selectedNode.type === "EMENDA") && (
            <>
              <SheetHeader className="pb-4 mb-4 border-b border-yellow-900 border-dashed relative">
                ...
                <Badge variant="outline" className="w-fit mb-2 text-xs uppercase rounded-none border bg-yellow-900/30 text-yellow-500 border-yellow-500 pr-4">
                  {selectedNode.data.label?.startsWith('EMENDA') ? 'EMENDA PARLAMENTAR' : 'CONTRATO FEDERAL'}
                </Badge>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="p-3 border bg-yellow-950/10 border-yellow-900/30 text-yellow-500 text-center">
                    ...
                    {selectedNode.data.label?.startsWith('EMENDA') && selectedNode.data.valorPago !== undefined && (
                      <p className="text-xs mt-2 opacity-80 uppercase tracking-widest pt-2 border-t border-yellow-900/30">
                        STATUS: {selectedNode.data.valorPago > 0 ? `Pagamento de R$ ${Number(selectedNode.data.valorPago).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : 'Apenas Empenhado / Sem Pagamento'}
                      </p>
                    )}
                  </div>
                </div>

                {selectedNode.data.label?.startsWith('EMENDA') && selectedNode.data.tipo && (
                  <div>
                    <h3 className="text-xs uppercase font-bold text-yellow-600 mb-2 border-b border-yellow-900/50 pb-1 flex items-center gap-2">
                    ...
                    <div className="p-3 border bg-yellow-950/10 border-yellow-900/30 text-yellow-400 text-xs leading-relaxed uppercase tracking-wide">
                      <span className="opacity-60">TIPO:</span> {selectedNode.data.tipo}<br/>
                      <span className="opacity-60">ANO/EXERCÍCIO:</span> {selectedNode.data.ano || "N/A"}<br/>
                      {selectedNode.data.programa && <><span className="opacity-60">PROGRAMA:</span> {selectedNode.data.programa}</>}
                    </div>
                  </div>
                )}
                ...
                <div>
                  <h3 className="text-xs uppercase font-bold text-yellow-600 mb-2 border-b border-yellow-900/50 pb-1 flex items-center gap-2">
                    <FileText className="w-4 h-4" /> {selectedNode.data.label?.startsWith('EMENDA') ? 'Destinação / Função' : 'Objeto / Destinação'}
                  </h3>
                  <div className="p-3 border bg-yellow-950/10 border-yellow-900/30 text-yellow-400 text-xs leading-relaxed uppercase tracking-wide">
                    {selectedNode.data.objeto || "N/A"}
                    {selectedNode.data.subfuncao && <><br/><span className="opacity-60">SUBFUNÇÃO:</span> {selectedNode.data.subfuncao}</>}
                  </div>
                </div>

                {selectedNode.type === "EMENDA" && selectedNode.data.beneficiario && (
                  <div className="mt-6 pt-6 border-t border-slate-800">
                    ...
                    <div className="p-3 border bg-teal-950/10 border-teal-900/30 text-teal-400 text-xs leading-relaxed uppercase tracking-wide mb-3">
                      <span className="opacity-60">NOME:</span> {selectedNode.data.beneficiario.nome}<br/>
                      <span className="opacity-60">CNPJ:</span> {selectedNode.data.beneficiario.cnpj}<br/>
                      <span className="opacity-60">UF:</span> {selectedNode.data.beneficiario.uf}<br/>
                      {selectedNode.data.beneficiario.area && <><span className="opacity-60">POLÍTICA PÚBLICA:</span> {selectedNode.data.beneficiario.area}<br/></>}
                      {selectedNode.data.beneficiario.situacao && <><span className="opacity-60">SITUAÇÃO:</span> {selectedNode.data.beneficiario.situacao}</>}
                    </div>
                    ...
                    {beneficiaryContracts.length === 0 ? (
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const res = await fetch(`/api/investigar/contratos-beneficiario?cnpj=${selectedNode.data.beneficiario.cnpj}`);
                            if (res.ok) {
                              const json = await res.json();
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                          {beneficiaryContracts.map((c: any, idx: number) => (
                            <div key={idx} className="p-2 border border-slate-800 bg-slate-950/50 text-[11px] leading-relaxed font-mono">
                              <div className="flex justify-between items-start mb-1">
                                <span className="font-bold text-teal-400 text-[9px] bg-teal-950/50 px-1 border border-teal-900">
                                  {c.tipo === 'COMPRADOR' ? 'COMPRADOR/ÓRGÃO' : 'FORNECEDOR'}
                                </span>
                                <span className="text-slate-500 text-[9px]">{c.data ? new Date(c.data).toLocaleDateString('pt-BR') : ''}</span>
                              </div>
                              <p className="text-slate-300 font-bold uppercase tracking-wider line-clamp-1">{c.orgao}</p>
                              <p className="text-slate-400 mt-1 uppercase text-[10px] line-clamp-2">{c.objeto}</p>
            ...
            <>
              <SheetHeader className="pb-4 mb-4 border-b border-teal-900 border-dashed relative">
                <VisuallyHidden>
                  ...
                  <SheetDescription>Visão geral de todas as emendas.</SheetDescription>
                </VisuallyHidden>
                <Badge variant="outline" className="w-fit mb-2 text-xs uppercase rounded-none border bg-teal-900/30 text-teal-500 border-teal-500 pr-4">
              ...
              <div className="space-y-6">
                <div className="pb-2 border-b border-teal-900/50">
                  ...
                  >
                    {selectedNode.data?.isExpanded ? "Recolher Emendas no Canvas" : "Ver Todas as Emendas no Canvas"}
                  </Button>
                </div>
                <div>
                  ...
                  <div className="p-3 border bg-teal-950/10 border-teal-900/30 text-teal-400">
                    <div className="flex justify-between items-end mb-2">
                      <span className="text-xs uppercase opacity-70">Total Empenhado</span>
                      <span className="text-lg font-bold tracking-widest">
                        R$ {Number(selectedNode.data.totalEmpenhado || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </span>
                    </div>
                    <div className="flex justify-between items-end mb-3">
                      <span className="text-xs uppercase opacity-70">Total Pago</span>
                      <span className="text-md font-bold text-teal-300">
                        R$ {Number(selectedNode.data.totalPago || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </span>
                    </div>
                    
                    <div className="w-full bg-teal-950 h-2 mt-4 relative">
                      <div className="absolute top-0 left-0 h-full bg-teal-500" style={{ width: `${selectedNode.data.percentualExecucao || 0}%` }}></div>
                    </div>
                    <p className="text-xs mt-2 uppercase tracking-widest text-right">
                      TAXA DE EXECUÇÃO: {selectedNode.data.percentualExecucao || 0}%
                    </p>
                  </div>
                </div>

                {(selectedNode.data.fantasmas > 0 || selectedNode.data.emendasPIX > 0) && (
                  <div>
                    <h3 className="text-xs uppercase font-bold text-orange-600 mb-2 border-b border-orange-900/50 pb-1 flex items-center gap-2">
                      <ShieldAlert className="w-4 h-4" /> Fatores de Risco Detectados
                    </h3>
                    <div className="p-3 border bg-orange-950/10 border-orange-900/30 text-orange-400 text-xs leading-relaxed uppercase tracking-wide">
                      {selectedNode.data.fantasmas > 0 && <span className="block mb-1">{selectedNode.data.fantasmas} Emenda(s) Fantasma(s) - Sem Pagamentos</span>}
                      {selectedNode.data.emendasPIX > 0 && <span className="block text-red-400">{selectedNode.data.emendasPIX} Emenda(s) PIX/Orçamento Secreto</span>}
                    </div>
                  </div>
                )}
                ...
                {Object.keys(selectedNode.data.porTipo || {}).length > 0 && (
                  <div>
                    ...
                    <ul className="text-xs space-y-1">
                      {Object.entries(selectedNode.data.porTipo).map(([tipo, qtd]) => (
                        <li key={tipo} className="flex justify-between text-teal-400/80">
                ...
                {selectedNode.data.topLocalidades && selectedNode.data.topLocalidades.length > 0 && (
                  <div>
                    <h3 className="text-xs uppercase font-bold text-teal-600 mb-2 border-b border-teal-900/50 pb-1">
                      Principais Localidades (Valores)
                    </h3>
                    <ul className="text-xs space-y-2">
                      {selectedNode.data.topLocalidades.map((loc: any, idx: number) => (
                        <li key={idx} className="flex flex-col text-teal-400/70 border-b border-teal-900/20 pb-1">
                          <span className="font-bold">{loc.localidade}</span>
                          <span>R$ {Number(loc.valor || 0).toLocaleString('pt-BR')}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
            ...
            <>
              <SheetHeader className="pb-4 mb-4 border-b border-blue-900 border-dashed relative">
                <VisuallyHidden>
                  ...
                  <SheetDescription>Informações detalhadas sobre a pessoa jurídica.</SheetDescription>
                </VisuallyHidden>
                <Badge variant="outline" className="w-fit mb-2 text-xs uppercase rounded-none border bg-blue-900/30 text-blue-500 border-blue-500 pr-4">
                  PESSOA JURÍDICA
                </Badge>
              ...
              <div className="space-y-6">
                {selectedNode.data.cnae && (
                  <div>
                    <h3 className="text-xs uppercase font-bold text-slate-400 mb-2 border-b border-slate-800 pb-1">CNAE Principal</h3>
                    <p className="text-xs text-slate-300">{selectedNode.data.cnae}</p>
                  </div>
                )}
                ...
                {!(selectedNode.data.label?.toUpperCase().includes('ELEICAO') || 
                  ...
                  <>
                    <div className="mt-8 pt-6 border-t border-slate-800">
                      ...
                      <p className="text-xs text-slate-500 mb-3 leading-relaxed">
                        Verifique indícios de empresa de fachada através do endereço de registro.
                      </p>
                      <a
                        href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
                          [selectedNode.data.label, selectedNode.data.municipio, selectedNode.data.uf, selectedNode.data.cnpj ? `CNPJ ${selectedNode.data.cnpj}` : null, 'Brasil'].filter(Boolean).join(" ")
                        )}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex w-full items-center justify-center p-3 border bg-blue-950/20 text-blue-400 border-blue-900 hover:bg-blue-900 hover:text-blue-300 rounded-none text-xs font-bold uppercase tracking-widest transition-all duration-300 group"
                      >
                        <MapIcon className="mr-2 h-4 w-4" /> Analisar Endereço (Street View)
                      </a>
                    </div>
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-800">
                      ...
                      <p className="text-xs text-slate-500 mb-3 leading-relaxed">
                        Extrair o Quadro de Sócios (QSA) e conexões desta empresa.
                      </p>
                      {!expandedNodes[selectedNode.id] ? (
                        <Button
                          variant="outline"
                          ...
                          onClick={() => {
                            setExpandedNodes(prev => ({ ...prev, [selectedNode.id]: true }));
                            handlePivotCNPJ(selectedNode.data.cnpj, selectedNode.id);
                          }}
                        >
                          <Search className="mr-2 h-4 w-4" /> Expandir Teia Societária
                        </Button>
                      ) : (
                        <div className="w-full text-center p-3 border border-green-900/50 bg-green-950/10 text-green-500/80 text-xs font-mono uppercase tracking-widest">
                          [ DRILLDOWN CONCLUÍDO NO CANVAS ]
            ...
            <>
              <SheetHeader className="pb-4 mb-4 border-b border-purple-900 border-dashed relative">
                <VisuallyHidden>
                  ...
                  <SheetDescription>Informações detalhadas sobre a pessoa física listada no QSA.</SheetDescription>
                </VisuallyHidden>
                <Badge variant="outline" className="w-fit mb-2 text-xs uppercase rounded-none border bg-purple-900/30 text-purple-400 border-purple-500 pr-4">
                  PESSOA FÍSICA (QSA)
                </Badge>
              ...
              <div className="space-y-6">
                <div className="mt-8 pt-6 border-t border-slate-800">
                  ...
                  <p className="text-xs text-slate-500 mb-3 leading-relaxed">
                    Rodar script de busca para localizar outras empresas onde {selectedNode.data.label} também é sócio(a).
                  </p>
                  {!expandedNodes[selectedNode.id] ? (
                    <Button
                      variant="outline"
                      ...
                      onClick={() => {
                        setExpandedNodes(prev => ({ ...prev, [selectedNode.id]: true }));
                        handleSocioSearch(selectedNode.data.label, selectedNode.id);
                      }}
                    >
                  ...
                  <p className="text-xs text-purple-900/40 mt-2 text-center">* Essa ação quebra sigilo através de cruzamentos com fontes governamentais externas.</p>
                </div>
              </div>
            </>
          )}
      ...
      {!isMobile ? (
        <Sheet open={dashboardOpen} onOpenChange={setDashboardOpen}>
          <SheetContent className="bg-black border-l-2 border-indigo-500 font-mono text-indigo-300 overflow-y-auto w-[480px] sm:w-[560px] pt-12 pr-6">
            <VisuallyHidden><SheetTitle>Raio-X de Gastos</SheetTitle><SheetDescription>Dashboard de Cota de Gabinete</SheetDescription></VisuallyHidden>
            <DashboardCotaConteudo nome={dashboardNome} data={dashboardData} loading={dashboardLoading} />
          </SheetContent>
        </Sheet>
      ) : (
