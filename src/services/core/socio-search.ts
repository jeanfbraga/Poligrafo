...
export async function buscarEmpresasDoSocio(nomeSocio: string) {
	...
	console.log(
		`[OSINT QSA] Buscando ${nomeNorm} usando mecanismo gratuito Dorking (DuckDuckGo + BrasilAPI)...`,
	);

	try {
		const query = encodeURIComponent(
			`"quadro de sócios" OR "qsa" "${nomeNorm}" cnpj`,
		);
		...
		const cnpjsEncontrados = [
			...new Set(matches.map((c) => c.replace(/\D/g, ""))),
		];
		...
		console.log(
			`[OSINT QSA] Extraídos ${cnpjsEncontrados.length} CNPJs. Validando QSA na BrasilAPI em paralelo...`,
		);
		...
		const validacoes = cnpjsEncontrados.slice(0, 6).map(async (cnpj) => {
			try {
				...
				if (resBrasil.ok) {
					...
					const isSocio = emp.qsa?.some((s: any) =>
						normalizeStringLocal(s.nome_socio).includes(nomeNorm),
					);
