...
export async function buscarDiariosMunicipais(
	options: BuscarDiariosOptions,
): Promise<ResultadoQueridoDiario> {
	const {
		termo,
		territoryIds,
		since,
	...
	try {
		...
		if (!response.ok) {
			console.warn(
				`[QUERIDO DIARIO] HTTP ${response.status} ao buscar "${termo}"`,
			);
		...
		if (e.name === "AbortError") {
			console.warn(
				`[QUERIDO DIARIO] Timeout (${timeout}ms) buscando "${termo}"`,
			);
		} else {
