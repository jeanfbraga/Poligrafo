...
import { DOU_SECTIONS, DOU_PERIODS } from './types';
...
export function extractJsonFromHtml(html: string): { jsonArray?: any[]; total?: number } {
 ...
 */
export async function buscarDOU(options: BuscarDOUOptions): Promise<ResultadoDOU> {
  const {
    termo,
    secao = 'TODOS',
    periodo = 'MES',
 ...
 */
export async function lerPublicacaoDOU(urlTitulo: string): Promise<PublicacaoDOU | null> {
 ...
 */
export async function buscarNomeacoesDOU(nomeCompleto: string, periodo = 'ANO'): Promise<ResultadoDOU> {
  return buscarDOU({
