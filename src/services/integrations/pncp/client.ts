 ...
 */
export async function fetchContratosByCNPJ(cnpj: string, yearsToFetch = 8): Promise<PNCPContract[]> {
  ...
  for (let year = currentYear; year > currentYear - yearsToFetch; year--) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(`[PNCP] Falha ao buscar contratos para ${cleanCnpj} no ano ${year}. Status: ${response.status}`);
        continue;
      }
      ...
      console.error(`[PNCP] Erro de rede ao buscar contratos para ${cleanCnpj} no ano ${year}`, error);
    }
  }
