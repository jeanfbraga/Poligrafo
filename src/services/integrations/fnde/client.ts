...
export async function consultarPNAE(
	municipio: string,
	uf: string,
	ano?: number,
): Promise<PnaeRepasse[]> {
	try {
		...
		if (municipio)
			filters.push(`contains(Municipio,'${municipio.toUpperCase()}')`);
...
export async function consultarFUNDEB(
	municipio: string,
	uf: string,
	ano?: number,
): Promise<FundebMatricula[]> {
	try {
		const filters = [];
		if (ano) filters.push(`AnoCenso eq ${ano}`);
		if (uf) filters.push(`Uf eq '${uf.toUpperCase()}'`);
		if (municipio)
			filters.push(`contains(MunicipioGe,'${municipio.toUpperCase()}')`);
...
export async function consultarPNATE(
	uf: string,
	municipio?: string,
): Promise<PnateRepasse[]> {
	try {
		const filters = [];
		if (uf) filters.push(`Uf eq '${uf.toUpperCase()}'`);
		if (municipio)
