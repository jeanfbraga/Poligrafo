...
// Subcomponentes modulares para reduzir complexidade ciclomática
function BillReaderLoading() {
  return (
...
function BillReaderError({ error, onBack }: { error: string; onBack: () => void }) {
  return (
...
function BillAutores({ autores }: { autores: any[] }) {
  const hasAutores = autores && autores.length > 0;
  return (
    <div className="border border-green-500/30 bg-green-950/10 p-4">
      ...
      {hasAutores ? (
        <ul className="space-y-1">
          {autores.map((autor: any, idx: number) => (
            <li key={idx} className="text-green-400 text-sm">
              {autor.nome} {autor.tipo ? `(${autor.tipo})` : ""}
            </li>
          ))}
        </ul>
      ) : (
...
function BillSituacao({ projeto }: { projeto: any }) {
  return (
    <div className="border border-green-500/30 bg-green-950/10 p-4">
      <h3 className="text-green-500 flex items-center gap-2 font-bold uppercase text-xs tracking-widest mb-3">
...
function BillTramitacoes({ tramitacoes }: { tramitacoes: any[] }) {
  const [showFullTimeline, setShowFullTimeline] = useState(false);
  const items = [...tramitacoes].reverse();
  const visibleItems = showFullTimeline ? items : items.slice(0, 3);
  const temMais = items.length > 3;
  ...
  return (
    ...
    >
      <div className="relative space-y-8 py-4">
        ...
        {visibleItems.map((tram: any, idx: number) => (
...
function BillTopNav({ onBack }: { onBack: () => void }) {
  return (
...
function BillMainCard({
  ...
  const dataApresentacao = projeto.data_apresentacao ? new Date(projeto.data_apresentacao).toLocaleDateString("pt-BR") : "";
    ...
    <TerminalWindow className="p-4 md:p-8 border-green-500/50" scanline={false}>
      ...
      {dataApresentacao && (
        <p className="text-green-400/80 font-mono text-xs md:text-sm mb-6 border-b border-green-500/20 pb-4">
          Apresentado em {dataApresentacao}
        </p>
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <BillAutores autores={projeto.autores_json} />
        <BillSituacao projeto={projeto} />
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-6">
        <button 
          onClick={onSummarize}
          disabled={aiLoading || Boolean(aiSummary)}
...
function BillIframeSection({ textoIntegral }: { textoIntegral: string }) {
  return (
...
export default function BillReaderDashboard({ idDeputado, idProjeto }: { idDeputado: string; idProjeto: string }) {
  ...
  if (loading) return <BillReaderLoading />;
  if (error || !projeto) return <BillReaderError error={error} onBack={() => router.push(`/perfil/deputado/${idDeputado}`)} />;

  const temTramitacoes = projeto.tramitacoes_json && projeto.tramitacoes_json.length > 0;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-black text-green-500 font-mono overflow-x-hidden relative">
      ...
      <div className="p-4 md:p-8">
        <BillTopNav onBack={() => router.push(`/perfil/deputado/${idDeputado}`)} />
        ...
        <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-700">
          ...
          {temTramitacoes && (
            <BillTramitacoes tramitacoes={projeto.tramitacoes_json} />
          )}

          {projeto.texto_integral && (
            <BillIframeSection textoIntegral={projeto.texto_integral} />
          )}
        </div>
      </div>
    </div>
