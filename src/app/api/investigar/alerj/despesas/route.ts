...
export async function GET(request: Request) {
	...
	const stream = new ReadableStream({
		async start(controller) {
			...
			try {
				sendEvent("STATUS", {
					msg: `Deep Dive ALERJ: Consultando despesas do parlamentar ${nome}...`,
				});

				// 1. Aciona o scraper do Playwright para buscar as despesas cruas
				const despesasCruas = await buscarDespesasDeputadoEstadualRJ(
					nome,
					sendEvent,
				);
				...
				if (despesasCruas.length === 0) {
					sendEvent("STATUS", {
						msg: `Nenhuma despesa recente encontrada no portal da ALERJ.`,
					});
				...
				sendEvent("STATUS", {
					msg: `Recuperando doadores de campanha para triagem heurística...`,
				});
				const doadores = await buscarDoadoresTSE(nome, "RJ");

				// 3. Aplica o Gemini
				sendEvent("STATUS", {
					msg: "[POLÍGRAFO IA] Triando gastos da ALERJ em busca de indícios ilícitos...",
				});
				const despesasAvaliadas = await analisarLoteComInteligencia(
					despesasCruas,
					"RJ",
					doadores,
					"Estadual",
				...
				sendEvent("DONE", {
					msg: `Operação de Deep Dive concluída: ${despesasAvaliadas.length} notas processadas.`,
				});
			} catch (err: any) {
				console.error("[ERRO ALERJ DEEP DIVE]", err);
				sendEvent("ERROR", {
					mensagem:
						err.message || "Falha ao processar o Portal da Transparência.",
				});
			} finally {
				safeClose();
			}
