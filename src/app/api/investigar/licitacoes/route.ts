...
export async function GET(request: Request) {
  ...
  if (!cleanCnpj || cleanCnpj.length !== 14) {
    return NextResponse.json({ error: 'CNPJ válido contendo 14 dígitos é obrigatório' }, { status: 400 });
  }

  const politico = politicoBruto ? politicoBruto.replace(/[^a-zA-Z0-9\sÁ-ÿ\-]/g, '').trim() : null;
  ...
  try {
    ...
    let aiAnalysis = null;
    if (politico) {
      aiAnalysis = await analisarComIAPNCP(cleanCnpj, politico, contratosParaIA);
    }
    ...
    return NextResponse.json({ error: 'Falha ao buscar dados no PNCP' }, { status: 500 });
  }
}
