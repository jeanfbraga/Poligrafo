...
describe("Cache Hit Validation", () => {
	...
	test("REJEITA: nodes é undefined", () => {
		expect(
			isCacheValid({ grafo_dados: { timestamp: "2026" } }, null),
		).toBeFalsy();
	});

	test("REJEITA: nodes não é array", () => {
		expect(isCacheValid({ grafo_dados: { nodes: "invalid" } }, null)).toBe(
			false,
		);
	});
	...
	test("ACEITA: cache completo com múltiplos nodes", () => {
		const cacheData = {
			grafo_dados: {
				...
				nodes: [
					{
						id: "pessoa-12345678900",
						type: "PESSOA",
						data: { label: "Paulo Abi Ackel", cargo: "Deputado Federal" },
					},
					{
						id: "emenda-0",
						type: "CONTRATO",
						data: { label: "EMENDA: Saúde", valor: 500000 },
					},
					{
						id: "bem-1",
						type: "CONTRATO",
						data: { label: "BEM DECLARADO: Apartamento", valor: 800000 },
					},
				],
				escopo: "CAMARA",
				partial: false,
