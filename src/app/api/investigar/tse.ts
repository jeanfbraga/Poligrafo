...
export function matchPalavraInteira(texto: string, palavra: string): boolean {
    ...
    const regex = new RegExp(`(?:^|\\s|-)${palavra.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:\\s|-|$)`);
    return regex.test(texto);
}

export async function fetchWithTimeout(resource: string | URL | RequestInfo, options: RequestInit & { timeout?: number } = {}) {
...
): Promise<{ cpf: string, documentoPrincipal: string, cnpjCampanha: string | null, isCnpj: boolean, municipio: string, idUe: string, nome?: string, idTse?: number, anoEleicao?: number, idEleicao?: string, patrimonioTotal?: number, bensDeclarados?: any[] } | null> {
    console.log(`[>> TSE ENTRY] buscarCpfNoTSE chamado: ${nomePolitico} UF:${uf} Cargo:${cargoCodigo}`);
    try {
        ...
        for (const eleicao of campanhas) {
            try {
                if (!isMunicipal) {
                    console.log(`[TSE DEBUG] Buscando "listar" TODOS os candidatos para ${uf} (eleição ${eleicao.ano}, cargo ${cargoCodigo})...`);
                    ...
                    let resListagem;
                    try { resListagem = await fetchWithTimeout(urlListagem, { timeout: 6000 }); } catch (e) { continue; }
                    if (!resListagem || !resListagem.ok) continue;
                    ...
                    if (candidatos.length > 0) {
                        ...
                        if (!match) {
                            const parts = termoNorm.split(/\s+/).filter((p: string) => !['de', 'da', 'do', 'dos', 'das'].includes(p));
                            match = candidatos.find((c: any) => {
                                const cUrna = normalizeString(c.nomeUrna || '');
                                const cNome = normalizeString(c.nomeCompleto || '');
                                return parts.every((p: string) => matchPalavraInteira(cUrna, p) || matchPalavraInteira(cNome, p));
                            });
                        }
                        ...
                        if (match && match.id) {
                            return await extrairDetalhesDoTSE(eleicao, uf, match, uf, nomePolitico);
                        }
                    }
                } else {
                    ...
                    let dataMuni;
                    try { dataMuni = await resMuni.json(); } catch (e) { continue; }
                    if (!dataMuni.municipios) continue;

                    const locais = dataMuni.municipios
                        .sort((a: any, b: any) => (a.codigo === '71072' || a.codigo === '60011' ? -1 : 1))
                        .map((m: any) => m.codigo);

                    console.log(`[TSE DEBUG] Buscando "listar" "${nomePolitico}"... Capital separada, restante em chunks.`);
                    ...
                    if (capitalLocal) {
                        try {
                            console.log(`[TSE DEBUG] Buscando na capital (${capitalLocal}) isoladamente...`);
                            ...
                            const resListagemCap = await fetchWithTimeout(urlListagemCap, { timeout: 15000 });
                            if (resListagemCap.ok) {
                                let dataListagemCap;
                                try { dataListagemCap = await resListagemCap.json(); } catch (e) { }
                                ...
                                if (candidatosCap.length > 0) {
                                    ...
                                    if (match && match.id) {
                                        console.log(`[TSE DEBUG] Alvo encontrado na capital (${capitalLocal}).`);
                                        const finalResult = await extrairDetalhesDoTSE(eleicao, capitalLocal, match, uf, nomePolitico);
                                        if (finalResult) return finalResult;
                                    }
                                }
                            }
                        } catch (e) {
                            console.warn(`[TSE DEBUG] Timeout/Erro na busca isolada da capital:`, e);
                        }
                    }
                    ...
                    for (let i = 0; i < locaisRestantes.length; i += chunkSize) {
                        ...
                        console.log(`[TSE DEBUG] Processando chunk ${Math.floor(i / chunkSize) + 1} (${chunk.length} municípios)...`);
                        ...
                        const chunkPromises = chunk.map(async (localidade: string) => {
                            try {
                                ...
                                const resListagem = await fetchWithTimeout(urlListagem, { timeout: 3500 });
                                if (resListagem.ok) {
                                    let dataListagem;
                                    try { dataListagem = await resListagem.json(); } catch (e) { return null; }
                                    const candidatos = dataListagem.candidatos || [];
                                    if (candidatos.length > 0) {
                                        const termoNorm = normalizeString(nomePolitico);
                                        // ETAPA 1: Busca apenas correspondência EXATA
                                        ...
                                        if (match && match.id) return { match, localidade };
                                    }
                                }
                            } catch (e) { }
                            return null;
                        ...
                        if (resultFound) {
                            console.log(`[TSE DEBUG] Alvo encontrado no município ${resultFound.localidade}.`);
                            const finalResult = await extrairDetalhesDoTSE(eleicao, resultFound.localidade, resultFound.match, uf, nomePolitico);
                            if (finalResult) return finalResult;
                        }
                    }
                }
        ...
        console.warn(`[TSE] Erro principal tentar resgatar o CPF de ${nomePolitico}:`, e);
        return null;
    }
}

async function extrairDetalhesDoTSE(eleicao: any, localidade: string, match: any, uf: string, nomePolitico: string) {
    console.log(`[TSE DEBUG] ID encontrado: ${match.id} (${match.nomeUrna}). Buscando detalhes em ${localidade}...`);
    ...
    const cnpjCampanha = jsonCpf?.cnpjcampanha ? jsonCpf.cnpjcampanha.replace(/\D/g, '') : null;
    const documentoValido = cpfReal || cnpjCampanha;
    const isCnpj = !cpfReal && !!cnpjCampanha;

    if (documentoValido) {
        const nomeMunicipioRaw = jsonCpf?.localCandidatura || jsonCpf?.unidadeEleitoral?.nome || uf;
        ...
        console.log(`[TSE DEBUG] SUCESSO! Documento Extraído. isCnpj=${isCnpj} Patrimônio: ${patrimonioTotal}`);
        return {
            cpf: documentoValido,
    ...
    console.log(`[TSE DEBUG] Falha: CPF e CNPJ de Campanha não encontrados nos detalhes do candidato.`);
    return null;
}
...
export async function buscarDoadoresTSE(
    ...
    try {
        ...
        if (!cacheErr && cacheData && cacheData.doadores && cacheData.doadores.length > 0) {
            console.log(`[TSE DEBUG] Doadores recuperados do cache Supabase (bypass WAF) para ${nomePolitico}.`);
            return cacheData.doadores;
        }

        // 1. Busca o ID e Partido do candidato
        const ano = idEleicao === '2045202024' ? '2024' : (idEleicao === '2040602022' ? '2022' : '2020');
        ...
        const candidato = dataBusca.candidatos?.find((c: any) => {
            const cUrna = normalizeString(c.nomeUrna || '');
            const cNome = normalizeString(c.nomeCompleto || '');
            return cUrna === termoNorm || cNome === termoNorm || matchPalavraInteira(cUrna, termoNorm) || matchPalavraInteira(cNome, termoNorm);
        });

        if (!candidato || !candidato.id) {
            console.log(`[TSE DEBUG] Candidato não encontrado na listagem para buscar doadores.`);
            return [];
        }
        ...
        console.log(`[TSE DEBUG] Buscando contas/ranking (WAF Bypass): ${urlContas}`);
        ...
        if (!resContas.ok) {
            console.log(`[TSE DEBUG] WAF/TSE bloqueou ou não encontrou contas: HTTP ${resContas.status}`);
            return [];
        }
        ...
        try {
            ...
            console.warn(`[TSE DEBUG] WAF bloqueou a busca de contas (Resposta não-JSON)`);
            return [];
        }

        // Extrai CPFs e CNPJs do ranking de doadores e remove nulos
        const listaDoadores = (dataContas.rankingDoadores || []).map((doacao: any) => {
        ...
        console.log(`[TSE DEBUG] ${doadoresUnicos.length} doadores únicos capturados! Salvando no cache Supabase...`);
        ...
        console.warn(`[TSE] Falha severa ao buscar doadores para ${nomePolitico}:`, e);
        return [];
    }
}
