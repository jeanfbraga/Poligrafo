...
> {
	...
	console.log(
		`[>> MUNICIPAL CE ENTRY] buscarMunicipalCE chamado para: ${nomeBuscado}`,
	);
...
export async function buscarCodigoMunicipioCE(
	...
	if (!municipiosCache) {
		try {
			const res = await fetchWithTimeout(`${API_BASE}/municipios`, {
				timeout: TIMEOUT_CEARA,
			});
			...
			data.forEach((m: any) => {
				if (m.nome_municipio && m.codigo_municipio) {
					municipiosCache![m.nome_municipio.toLowerCase().trim()] = String(
						m.codigo_municipio,
					);
				}
			});
...
export async function buscarContratosTceCE(
...
export async function buscarEmpenhosTceCE(
	codigoMunicipio: string,
	anoMesRef: string,
	codigoOrgao: string = "02",
): Promise<any[]> {
	...
	try {
		...
		return items.map((e: any) => ({
			...
			historico:
				`${e.historico1_empenho || ""} ${e.historico2_empenho || ""}`.trim(),
		}));
	} catch (e) {
...
export async function buscarDespesasMunicipalCE(
	...
	if (!municipioUri) {
		console.log(
			`[TCE-CE] Redirecionando ${identificador} para Proxy OSINT (Faltou URI Geográfica).`,
		);
	...
	console.log(
		`[TCE-CE] Iniciando extração nativa para ${casa} de ${municipioUri}`,
	);

	const codigoMunicipio = await buscarCodigoMunicipioCE(
		municipioUri.replace(/-/g, " "),
	);
	if (!codigoMunicipio) {
		console.warn(
			`[TCE-CE] Município ${municipioUri} não localizado. Caindo pro Proxy.`,
		);
		const payload = await buscarProxyOsint(identificador, nomeParaBusca);
		return payload.despesasFederais;
	}
	...
	console.log(
		`[TCE-CE] Extração concluída. Total de docs nativos para IA: ${formatados.length}`,
	);
	return formatados;
}
