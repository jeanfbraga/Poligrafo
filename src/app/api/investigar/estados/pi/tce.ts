...
export async function buscarIdPrefeituraPI(nomeMunicipio: string): Promise<number | null> {
    const nomeLimpo = nomeMunicipio.toLowerCase().trim();

    if (!prefeiturasCachePI) {
        try {
            const res = await fetchWithTimeout(`${API_BASE}/prefeituras`, { timeout: TIMEOUT_PI });
...
export async function buscarCredoresPI(idUnidade: number, exercicio: number): Promise<any[]> {
...
export async function buscarDespesasPI(municipioNome: string, casa: string): Promise<any[]> {
    console.log(`[TCE-PI] Iniciando extração nativa para ${casa} de ${municipioNome}`);
    
    const idPrefeitura = await buscarIdPrefeituraPI(municipioNome.replace(/-/g, ' '));
    if (!idPrefeitura) {
    ...
    if (!credores || credores.length === 0) {
        console.log(`[TCE-PI] Sem credores em ${anoAtual}. Tentando ${anoAtual - 1}...`);
        credores = await buscarCredoresPI(idPrefeitura, anoAtual - 1);
    }
    if (!credores || credores.length === 0) {
        console.log(`[TCE-PI] Sem credores em ${anoAtual - 1}. Tentando ${anoAtual - 2}...`);
    ...
    console.log(`[TCE-PI] Extração concluída. Total de credores para IA: ${formatados.length}`);
    return formatados;
}
