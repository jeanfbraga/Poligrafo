...
export async function buscarDespesasDeputadoEstadualSP(
	...
	const _matricula = identificador; // A ALESP usa a matrícula, não o CPF para despesas
	...
	try {
		for (const mes of meses) {
			...
			if (!response.ok) {
				...
				if (mes === meses[0] && sendEvent) {
					// Só manda 1 vez pra não poluir
		...
		if (sendEvent) {
			sendEvent("API_WARNING", {
				fonte: "Assembleia Legislativa de SP (ALESP)",
				mensagem:
					"O servidor da AL-SP recusou a conexão ou deu timeout. Tente novamente mais tarde.",
			});
		}
		return [];
	}
