...
function nomesBatem(nomeXml: string, alvoNorm: string, alvoTokens: string[]): boolean {
	const n = normalizaNome(nomeXml);
	if (!n) return false;
	if (n === alvoNorm || alvoNorm.includes(n) || n.includes(alvoNorm)) return true;
	const tokensXml = n.split(" ");
	const [menor, maior] = tokensXml.length <= alvoTokens.length ? [tokensXml, alvoTokens] : [alvoTokens, tokensXml];
	return menor.every((t) => maior.includes(t));
}

function extrairTagXml(bloco: string, tag: string): string {
	return bloco.match(new RegExp(`<${tag}>([^<]*)</${tag}>`))?.[1] || "";
}

function parseBlocoDespesa(
	bloco: string,
	anoMinimo: number,
	validadorNome: (nome: string) => boolean,
	urlXml: string,
): any | null {
	if (!bloco.includes("<Deputado>")) return null;
	const mNome = bloco.match(/<Deputado>([^<]*)<\/Deputado>/);
	if (!mNome || !validadorNome(mNome[1])) return null;
	...
	return {
		...
		dataDocumento: `${extrairTagXml(bloco, "Ano")}-${String(extrairTagXml(bloco, "Mes")).padStart(2, "0")}-01`,
		urlDocumento: urlXml,
	};
}

async function streamXmlDespesas(
	...
	while (true) {
		...
		while ((idx = buffer.indexOf("</despesa>")) !== -1) {
			...
			const desp = parseBlocoDespesa(bloco, anoMinimo, validadorNome, urlXml);
			if (desp) extraidas.push(desp);
			if (extraidas.length >= MAX_DESPESAS) break;
		}
		if (extraidas.length >= MAX_DESPESAS || bytesLidos >= MAX_BYTES) {
...
export async function buscarDespesasDeputadoEstadualSP(
	...
	const validadorNome = (nomeXml: string) => nomesBatem(nomeXml, alvoNorm, alvoTokens);

	try {
		const res = await fetch(urlXml, { signal: AbortSignal.timeout(60000) });
		if (!res.ok || !res.body) throw new Error(`HTTP ${res.status}`);

		const despesasExtraidas = await streamXmlDespesas(res.body.getReader(), validadorNome, anoMinimo, urlXml);
		...
		if (sendEvent) {
			sendEvent("API_WARNING", {
				fonte: "Assembleia Legislativa de SP (ALESP)",
				mensagem: "O repositório de dados da AL-SP recusou a conexão ou deu timeout. Tente novamente mais tarde.",
			});
		}
		return [];
	}
