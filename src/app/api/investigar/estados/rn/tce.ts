...
export async function buscarIdUnidadeRN(
	...
	if (!jurisdicionadosCacheRN) {
		try {
			const res = await fetchWithTimeout(
				`${API_BASE}/InformacoesBasicasApi/JurisdicionadosTCE/Json`,
				{ timeout: TIMEOUT_RN },
			);
			...
			(Array.isArray(data) ? data : []).forEach((j: any) => {
				if (j.nomeOrgao && j.identificadorUnidade) {
					jurisdicionadosCacheRN![j.nomeOrgao.toLowerCase().trim()] =
						j.identificadorUnidade;
				}
			});
		} catch (e) {
	...
	if (jurisdicionadosCacheRN[nomeLimpo])
		return jurisdicionadosCacheRN[nomeLimpo];
	for (const [key, val] of Object.entries(jurisdicionadosCacheRN)) {
		if (
			key.includes(nomeLimpo) ||
			nomeLimpo.includes(key.replace("prefeitura municipal de ", ""))
		) {
			return val;
		}
	}
...
export async function buscarDespesasRN(
	municipioNome: string,
	casa: string,
): Promise<any[]> {
	console.log(
		`[TCE-RN] Iniciando extração nativa para ${casa} de ${municipioNome}`,
	);

	const idUnidade = await buscarIdUnidadeRN(municipioNome.replace(/-/g, " "));
	if (!idUnidade) {
		console.warn(
			`[TCE-RN] Município ${municipioNome} não localizado nos jurisdicionados.`,
		);
		return [];
	}
	...
	console.log(
		`[TCE-RN] Extração concluída. Total de docs para IA: ${formatados.length}`,
	);
	return formatados;
}
