...
export async function buscarServidoresCMRJ(
	...
	console.log(
		`[CÂMARA RJ] Buscando servidores para o vereador: ${nomeVereador}`,
	);
	...
	try {
		...
		if (gabineteError || !gabineteData) {
			console.warn(
				`[CÂMARA RJ] Gabinete não encontrado para: ${nomeVereador}. Erro:`,
				gabineteError?.message,
			);
			return [];
		}
		...
		console.log(
			`[CÂMARA RJ] Encontrado gabinete para ${nomeVereador}: ${lotacao}`,
		);
		...
		if (servidoresError || !servidores) {
			console.warn(
				`[CÂMARA RJ] Falha ao buscar servidores do ${lotacao}:`,
				servidoresError?.message,
			);
			return [];
		}
		...
		console.log(
			`[CÂMARA RJ] ${servidoresRelacionados.length} servidores encontrados para ${nomeVereador}`,
		);
	} catch (error: any) {
		console.warn(
			`[CÂMARA RJ] Falha ao extrair servidores via Supabase:`,
			error.message,
		);
	}
