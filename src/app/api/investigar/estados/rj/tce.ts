...
export async function buscarMunicipalRJ(nomeBuscado: string): Promise<{
    ...
    console.log(`[>> MUNICIPAL RJ ENTRY] buscarMunicipalRJ chamado para: ${nomeBuscado}`);
    const resultados: {
        ref: string;
        id: string;
        nome: string;
...
import { buscarContratosTceRj, buscarComprasDiretasTceRj } from './tcerj-client';
...
export async function buscarDespesasVereadorRJ(
    ...
    if (resultados[0].status === 'fulfilled') {
        todasAsDespesas = todasAsDespesas.concat(resultados[0].value.despesasFederais || []);
    ...
    if (casa === 'CAMARA_MUNICIPAL_RJ' && nomeVereador) {
        try {
            ...
            if (!error && cotaDespesas && cotaDespesas.length > 0) {
                console.log(`[CMRJ-COTA] ${cotaDespesas.length} despesa(s) de cota de gabinete encontrada(s) para ${nomeVereador}`);
                ...
                console.log(`[CMRJ-COTA] Nenhuma despesa de cota encontrada ainda. ETL pendente.`);
            }
        } catch (err: any) {
