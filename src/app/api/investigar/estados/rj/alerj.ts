 ...
 */
export async function buscarPerfilDOCIGP(nomeDeputado: string, sendEvent?: any): Promise<{
    id: number;
    nome: string;
    apelido: string;
    partido: string;
    ...
    try {
        ...
        for (let page = 1; page <= 8; page++) {
            if (sendEvent) {
                sendEvent('STATUS', { msg: `Buscando dossiê no DOCIGP da ALERJ (página ${page} de 8)...` });
            }
            try {
                ...
                const match = data.find((c: any) => {
                    ...
                    if (nNorm.includes(termoNorm) || nickNorm.includes(termoNorm) ||
                        termoNorm.includes(nNorm) || termoNorm.includes(nickNorm)) return true;
                    ...
                    if (palavrasBusca.length >= 2 && palavrasDocigp.length >= 2) {
                        ...
                        if (palavrasDocigp.includes(primeiroBusca) && palavrasDocigp.includes(ultimoBusca)) return true;
                    }
                    return false;
                });
                ...
                if (match) {
                    ...
                    if (sendEvent) {
                        sendEvent('STATUS', { msg: `Dossiê encontrado no DOCIGP! (ID: ${match.id})` });
                    }
                    return {
                        id: match.id,
                        nome: match.name,
 ...
 */
export async function buscarDespesasDeputadoEstadualRJ(nomeDeputado: string, sendEvent?: any) {
    const despesasExtraidas: any[] = [];
    
    if (sendEvent) sendEvent('STATUS', { msg: `[OSINT ALERJ] Iniciando robô visual em 2º plano para buscar: ${nomeDeputado}...` });
    ...
    try {
        ...
        await page.goto('https://docigp.alerj.rj.gov.br/transparencia#/', { waitUntil: 'load', timeout: 60000 });
        
        if (sendEvent) sendEvent('STATUS', { msg: `[OSINT ALERJ] Acessado portal da transparência. Buscando perfil...` });
        ...
        const nameRow = page.locator(`tr.cursor-pointer:has-text("${lastName}")`).first();
        
        await nameRow.click();
        await page.waitForTimeout(4000);

        if (sendEvent) sendEvent('STATUS', { msg: `[OSINT ALERJ] Dossiê encontrado. Carregando documentos...` });
        ...
        for (const s of selects) {
            try {
                ...
                const maxOption = options.find(o => o.includes('250') || o.includes('100') || o.includes('Todos')) || options[options.length - 1];
        ...
        if (sendEvent) sendEvent('STATUS', { msg: `[OSINT ALERJ] Mês mais recente aberto. Buscando Lançamentos...` });
        ...
        if (sendEvent) sendEvent('STATUS', { msg: `[OSINT ALERJ] Iniciando varredura e extração de PDFs com suporte a paginação...` });

        while (hasNextPage && processedCount < maxProcessLimit) {
            const entriesTable = page.locator('table').nth(2);
            await entriesTable.waitFor({ state: 'visible', timeout: 10000 }).catch(() => {});
            const entryRows = await entriesTable.locator('tbody tr').all();
            
            for (let i = 0; i < entryRows.length && processedCount < maxProcessLimit; i++) {
                try {
                    ...
                    if (docsCount > 0) {
                        ...
                        const valorAbsoluto = Math.abs(parseFloat(valorStr.replace(/\./g, '').replace(',', '.')) || 0);
                        ...
                        await page.waitForSelector('[title="Visualizar documento"]', { state: 'visible', timeout: 8000 }).catch(() => {});
                        ...
                        if (valorAbsoluto > 0 && cnpjCpf.length >= 11) {
                            despesasExtraidas.push({
                                ...
                                nomeFornecedor: favorecidoStr.split('\n')[0].trim() || 'Fornecedor Identificado',
                                tipoDespesa: centroCusto.split('\n')[0].trim() || 'Verba de Gabinete DOCIGP',
                                valorDocumento: valorAbsoluto,
                                dataDocumento: dataStr.trim(),
                                urlDocumento: documentosExtraidos.length > 0 ? documentosExtraidos[0] : `https://docigp.alerj.rj.gov.br/transparencia#/`
                            });
                            processedCount++;
                            if (sendEvent) sendEvent('STATUS', { msg: `[OSINT ALERJ] NF Extraída: ${documentosExtraidos.length} docs (${processedCount}/${maxProcessLimit})` });
                        }
                    }
                } catch (err) {
            ...
            if (nextBtns.length > 0) {
                ...
                if (isEnabled) {
                    if (sendEvent) sendEvent('STATUS', { msg: `[OSINT ALERJ] Paginação: Indo para a próxima página de lançamentos...` });
                    await nextBtn.click();
                    await page.waitForTimeout(3000);
                } else {
                    hasNextPage = false;
        ...
        if (sendEvent) sendEvent('STATUS', { msg: `[OSINT ALERJ] Robô visual finalizado com sucesso.` });

    } catch (error: any) {
        console.error(`[ESTADUAL RJ] Erro no Robô Playwright: ${error?.message}`);
        if (sendEvent) sendEvent('STATUS', { msg: `[OSINT ALERJ ALERTA] Robô visual falhou: ${error?.message}` });
    } finally {
