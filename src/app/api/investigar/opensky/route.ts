...
export async function POST(req: Request) {
	try {
		...
		if (
			...
			!/^[a-fA-F0-9]{6}$/.test(icao24)
		) {
			return NextResponse.json(
				{
					error:
						"ICAO24 inválido (deve possuir exatamente 6 caracteres hexadecimais)",
				},
				{ status: 400 },
			);
		}
		...
		const nodes = voos.map((voo) => ({
			...
			data: {
				...
				valor: voo.baroAltitude
					? `${Math.round(voo.baroAltitude)}m altitude`
					: "N/A",
		...
		return NextResponse.json(
			{ error: "Erro interno ao buscar voos" },
			{ status: 500 },
		);
	}
}
