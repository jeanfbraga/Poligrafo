	...
	try {
		...
		if (allProcessos.length > 0) {
			for (let i = 0; i < allProcessos.length; i++) {
				...
				const assuntoPrinc =
					proc.assuntos?.[0]?.nome || "Improbidade Administrativa";
				...
				try {
					...
					const resumo = await traduzirJuridiquesSancoes([
						{
							titulo: assuntoPrinc,
							descricao: `Processo da Classe de Improbidade Administrativa nº ${proc.numeroProcesso}`,
						},
					]);
				...
				alertasPessoais.push(
					`[DATAJUD] Processo ${proc.numeroProcesso} (${proc.orgaoJulgador?.nome || "TJ"}): ${motivoFinal} (Risco ${gravidadeFinal})`,
				);
