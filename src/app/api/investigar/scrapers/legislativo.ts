...
export async function buscarPolitico(query: string): Promise<ParlamentarBasico | null> {
    try {
...
export async function buscarSenador(query: string): Promise<ParlamentarBasico | null> {
    try {
        const termoBusca = normalizeString(query);
        const url = `https://legis.senado.leg.br/dadosabertos/senador/lista/legislatura/57`;
        const res = await fetchWithTimeout(url, { timeout: 4500, headers: { 'Accept': 'application/json' } });
        ...
        const match = senadoresArray.find((s: any) =>
            normalizeString(s.IdentificacaoParlamentar.NomeParlamentar).includes(termoBusca) ||
            normalizeString(s.IdentificacaoParlamentar.NomeCompletoParlamentar).includes(termoBusca)
        );

        if (!match) return null;

        const mandatos = Array.isArray(match.Mandatos?.Mandato) ? match.Mandatos.Mandato : [match.Mandatos?.Mandato];
        const uf = mandatos[0]?.UfParlamentar || 'DF';

        const exercicios = Array.isArray(mandatos[0]?.Exercicios?.Exercicio) ? mandatos[0].Exercicios.Exercicio : [mandatos[0]?.Exercicios?.Exercicio];
        const afastado = exercicios.find((ex: any) => ex?.SiglaCausaAfastamento && !ex?.DataFim);
        let afastamentoDados;

        if (afastado) {
            const suplentes = Array.isArray(mandatos[0]?.Suplentes?.Suplente) ? mandatos[0].Suplentes.Suplente : [mandatos[0]?.Suplentes?.Suplente];
...
export async function buscarPoliticosCamaraLista(nome: string, ufScope?: string | null): Promise<ParlamentarBasico[]> {
    try {
...
export async function buscarSenadoresLista(nome: string, ufScope?: string | null): Promise<ParlamentarBasico[]> {
    try {
        ...
        const matches = senadoresArray.filter((s: any) => {
            const matchNome = normalizeString(s.IdentificacaoParlamentar?.NomeParlamentar || '').includes(termoBusca) ||
                              normalizeString(s.IdentificacaoParlamentar?.NomeCompletoParlamentar || '').includes(termoBusca);
        ...
        return matches.slice(0, 5).map((m: any) => {
            const mandatos = Array.isArray(m.Mandatos?.Mandato) ? m.Mandatos.Mandato : [m.Mandatos?.Mandato];
            const uf = mandatos[0]?.UfParlamentar || 'DF';
...
export async function buscarDetalhesPolitico(id: number): Promise<DetalhesDeputado | null> {
