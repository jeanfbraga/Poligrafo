...
export async function investigarFornecedorNivelHard(cnpj: string) {
	...
	const [resSancoes, resCompras, _resBrasil] = await Promise.allSettled([
		apiKey
			? fetchWithTimeout(
	...
	if (resCompras.status === "fulfilled" && resCompras.value.ok) {
		try {
			...
			if (contratos.length > 0) {
				alertas.push(
					`[COMPRAS.GOV] ${contratos.length} Contratos Federais Ativos.`,
				);
				penality += 10;
			}
		} catch (_e) {}
	...
	if (convenios && convenios.valorTotal > 0) {
		alertas.push(
			`[TRANSFEREGOV] Recebedor de ${convenios.quantidade} Convênio(s) Federal(is). Total: R$ ${convenios.valorTotal.toLocaleString("pt-BR")}`,
		);
		penality += 30;
	}
			...
			for (let i = 0; i < qsa.slice(0, 5).length; i++) {
				...
				try {
					...
					if (douRes.total > 0 && douRes.publicacoes.length > 0) {
						...
						const ehNomeacao =
							pub.tipoPublicacao?.toLowerCase().includes("nomeação") ||
							pub.titulo?.toLowerCase().includes("nomear") ||
							pub.titulo?.toLowerCase().includes("portaria");
					...
					if (qdRes.total_gazettes > 0 && qdRes.gazettes.length > 0) {
						...
						const trecho =
							gazette.excerpts && gazette.excerpts.length > 0
								? gazette.excerpts[0].substring(0, 150)
								: "";
						const isNomeacaoOuContrato =
							trecho.toLowerCase().includes("nomeaç") ||
							trecho.toLowerCase().includes("contrat") ||
							trecho.toLowerCase().includes("portaria");
				...
				try {
					...
					if (nepoMatch) {
						...
						const cargoStr =
							nepoMatch.cargo || nepoMatch.vinculo || "Cargo N/I";
