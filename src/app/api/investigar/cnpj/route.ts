...
export async function GET(request: Request) {
    ...
    if (!cnpjLimpo || cnpjLimpo.length !== 14 || !origemId) {
        return NextResponse.json({ error: 'Parâmetros ?cnpj (14 dígitos) e ?origemId válidos são obrigatórios.' }, { status: 400 });
    }
    ...
    const stream = new ReadableStream({
        async start(controller) {
            ...
            try {
                ...
                if (!empresa) {
                    sendEvent('ERROR', { mensagem: `Não foi possível localizar o CNPJ ${cnpj} nas bases públicas (BrasilAPI / ReceitaWS).` });
                    safeClose();
                    return;
                }
                ...
                if (apiKey) {
                    ...
                    try {
                        const resCompras = await fetchWithTimeout(`https://compras.dados.gov.br/contratos/v1/contratos.json?cnpj_contratada=${cnpjLimpo}`);
                        if (resCompras.ok) {
                    ...
                    try {
                        const resSancoes = await fetchWithTimeout(`https://api.portaldatransparencia.gov.br/api-de-dados/sancoes?cnpjSancionado=${cnpjLimpo}&pagina=1`, {
                            headers: { 'chave-api-dados': apiKey }
                        });
                        if (resSancoes.ok) {
                            const sancoes = await resSancoes.json();
                            if (Array.isArray(sancoes) && sancoes.length > 0) {
                                sendEvent('STATUS', { msg: `[ALERTA] Empresa consta no Cadastro de Sancionados da CGU! ${sancoes.length} registro(s).` });
                    ...
                    try {
                        const resConv = await fetchWithTimeout(`https://api.transferegov.gestao.gov.br/convenios?cnpj_convenente=${cnpjLimpo}`);
                        if (resConv.ok) {
                            ...
                            if (Array.isArray(convData) && convData.length > 0) {
                                ...
                                sendEvent('STATUS', { msg: `[ATENÇÃO] ${convData.length} convênio(s) federal(is). Valor total: R$ ${valorTotal.toLocaleString('pt-BR')}` });
                                sendEvent('NODE_NOVO', {
                    ...
                    try {
                        const resAnac = await fetchWithTimeout(`https://rab.api.aero/v1/aeronaves?proprietario=${encodeURIComponent(cnpjLimpo)}`);
                        if (resAnac.ok) {
