...
export function ShareDialog({
	...
	const [fotoBase64, setFotoBase64] = useState<string | null | undefined>(
		undefined,
	);
				...
				if (data.politicoFoto) {
					try {
						const res = await fetch(
							`/api/proxy-image?url=${encodeURIComponent(data.politicoFoto)}`,
						);
						if (res.ok) {
	...
	const hiddenCard = data && (
		...
		>
			<div
				ref={hiddenRef}
				className="w-270 h-480 bg-[#050505] p-12 font-mono flex flex-col relative overflow-hidden"
				style={{
					backgroundImage:
						"radial-gradient(circle, #002200 2px, transparent 2px)",
			...
			>
				...
				<div className="flex items-center gap-4 mb-16 pb-8 border-b-4 border-green-500/30">
					...
					<span className="text-5xl font-bold tracking-[0.3em] text-green-500 uppercase">
						POLÍGRAFO
					</span>
					<Badge
						variant="outline"
						className="text-2xl py-2 px-4 bg-green-900/50 text-green-400 border-green-500/50 rounded-none ml-4"
					>
						IA_OSINT
					</Badge>
				</div>
				...
				<div className="flex items-center gap-10 mb-16 bg-black/60 p-10 border border-green-500/50 shadow-[0_0_50px_rgba(34,197,94,0.15)]">
					{fotoBase64 || data.politicoFoto ? (
						<img
							src={fotoBase64 || data.politicoFoto || ""}
							alt="Foto"
							className="w-48 h-48 object-cover border-2 border-green-500 shrink-0 filter grayscale contrast-125"
						/>
					) : (
					...
					<div className="flex flex-col gap-4 w-full">
						<div className="flex gap-4">
							<Badge
								variant="outline"
								className="text-2xl uppercase bg-green-900/30 text-green-500 border-green-500 rounded-none p-3"
							>
								{data.politicoCargo}
							</Badge>
							<Badge
						...
						<h1 className="text-6xl font-bold uppercase tracking-wider text-green-400 wrap-break-word">
							{data.politicoNome}
						</h1>
					</div>
				</div>
				...
				<div className="flex-1 flex flex-col gap-6 relative z-10">
					...
					<div
						className={`p-12 border-l-12 bg-black/80 shadow-2xl relative overflow-hidden ${
							(data.achadoScore || 0) >= 85 || data.isFantasma
								? "border-red-600 shadow-[inset_0_0_100px_rgba(239,68,68,0.1)]"
								: (data.achadoScore || 0) >= 60
									? "border-yellow-500 shadow-[inset_0_0_100px_rgba(234,179,8,0.1)]"
									: "border-slate-500"
						}`}
					>
						<div
							className={`text-2xl font-bold mb-6 inline-block px-4 py-2 ${
								(data.achadoScore || 0) >= 85 || data.isFantasma
									? "bg-red-950/50 text-red-500 border border-red-500/50"
									: (data.achadoScore || 0) >= 60
										? "bg-yellow-950/50 text-yellow-500 border border-yellow-500/50"
						...
						<h2
							className={`text-5xl font-bold uppercase tracking-wide leading-tight mb-8 ${
								(data.achadoScore || 0) >= 85 || data.isFantasma
									? "text-red-400"
									: (data.achadoScore || 0) >= 60
										? "text-yellow-400"
						...
						{data.achadoValor !== undefined && (
							<div className="mb-10">
								...
								>
									R${" "}
									{data.achadoValor.toLocaleString("pt-BR", {
						...
						{(data.achadoMotivo || data.achadoAlerta) && (
							<div
								className={`mt-8 p-8 border-2 border-dashed ${
									(data.achadoScore || 0) >= 85 || data.isFantasma
		...
		<div className="flex flex-col gap-4 w-full">
			...
			<div className="grid grid-cols-3 gap-2 mt-2">
				<Button
					...
					className="bg-transparent border-green-900 text-green-500 hover:bg-green-900/50 h-10 rounded-none flex items-center justify-center"
				>
		...
		return (
			<>
				...
				<Sheet open={open} onOpenChange={onOpenChange}>
					<SheetContent
						side="bottom"
						className="bg-[#050505] border-t-2 border-green-500 font-mono p-6"
					>
						<SheetHeader className="mb-6 text-center">
							<VisuallyHidden>
								<SheetTitle>Compartilhar</SheetTitle>
							</VisuallyHidden>
							<h2 className="text-xl font-bold uppercase tracking-widest text-green-400 mb-2">
						...
						<div className="flex flex-col items-center gap-6">
							{isGenerating ? (
								<div className="h-75 w-full flex items-center justify-center border border-dashed border-green-900">
									<span className="text-green-500 animate-pulse text-xs uppercase tracking-widest">
										Processando Imagem...
									</span>
								</div>
							) : previewUrl ? (
								<img
									src={previewUrl}
									alt="Preview"
									className="max-h-75 object-contain border border-green-500/50 shadow-lg"
								/>
							) : null}
			...
			<Dialog open={open} onOpenChange={onOpenChange}>
				<DialogContent className="bg-[#050505] border border-green-500 font-mono text-green-500 max-w-4xl p-0 overflow-hidden shadow-[0_0_50px_rgba(34,197,94,0.15)] rounded-none">
					...
					<div className="flex h-150">
						{/* LEFT COLUMN: PREVIEW */}
						<div
							className="w-1/2 bg-black border-r border-green-900 p-6 flex flex-col relative items-center justify-center"
							style={{
								backgroundImage:
						...
						>
							{isGenerating ? (
								...
								<img
									src={previewUrl}
									alt="Preview"
									className="h-full w-auto object-contain border border-green-500/30"
								/>
							) : null}
						</div>
						...
						<div className="w-1/2 p-8 flex flex-col">
							...
							<div className="mt-auto">{shareActions}</div>
						</div>
					</div>
				</DialogContent>
			</Dialog>
