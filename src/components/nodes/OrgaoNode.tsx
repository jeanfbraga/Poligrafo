	...
	return (
		<>
			<Handle
				...
				className="bg-emerald-500! rounded-none! w-3 h-3 border-none!"
			/>
			<Card className="w-72 bg-black border-emerald-500 rounded-none font-mono text-emerald-400">
				<CardHeader className="flex flex-col gap-2 pb-2 border-b border-emerald-900">
					<Badge
						variant="outline"
						className="w-fit bg-emerald-950/30 text-emerald-400 border-emerald-500 rounded-none text-xs uppercase"
					>
						INSTITUIÇÃO PÚBLICA
					</Badge>
				...
				<CardContent className="pt-3 space-y-2">
					<div>
						...
						<p className="text-xs uppercase">{data.esfera}</p>
					</div>
					...
					{data.isSearching ? (
						<div className="mt-4 pt-3 border-t border-emerald-900/50">
							...
							<div className="w-full h-1 bg-emerald-950 overflow-hidden">
								<div
									className="h-full bg-emerald-500 w-1/3 animate-[slide_1.5s_ease-in-out_infinite]"
