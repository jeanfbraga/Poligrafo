...
export const SocioNode = ({ data }: { data: any }) => {
    return (
        <>
            <Handle type="target" position={Position.Top} className="!bg-purple-500 !rounded-none w-3 h-3 !border-none" />
            <Card 
            ...
            >
                <CardHeader className="flex flex-col gap-1 pb-2 border-b border-purple-900">
                    <div className="flex items-center gap-2">
                        ...
                        <CardTitle className="text-xs font-bold uppercase tracking-wider truncate" title={data.label}>{data.label}</CardTitle>
                    </div>
                </CardHeader>
                <CardContent className="pt-2">
                    <p className="text-xs uppercase font-bold text-purple-600">Cargo / Qualificação</p>
                    ...
                    {data.isSearching && (
                        <div className="mt-4 pt-3 border-t border-purple-900/50">
                            ...
                            <div className="w-full h-1 bg-purple-950 overflow-hidden">
                                <div className="h-full bg-purple-500 w-1/3 animate-[slide_1.5s_ease-in-out_infinite]" style={{ animationName: 'slideRight' }}></div>
                            </div>
                        </div>
                    )}
                </CardContent>
            </Card>
            <Handle type="source" position={Position.Bottom} className="!bg-purple-500 !rounded-none w-3 h-3 !border-none" />
        </>
    );
};
