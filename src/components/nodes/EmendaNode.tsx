...
import { Landmark, Share2, ShieldAlert } from "lucide-react";
...
export const EmendaNode = ({ data }: { data: any }) => {
	...
	const borderColor = isFantasma
		? "border-red-500"
		: risco === "CRÍTICO"
			? "border-orange-500"
			: "border-teal-500";
	const textColor = isFantasma
		? "text-red-400"
		: risco === "CRÍTICO"
			? "text-orange-400"
			: "text-teal-400";
	const barColor = isFantasma
		? "bg-red-500"
		: taxa < 30
			? "bg-yellow-500"
			: "bg-teal-500";
	...
	return (
		<div ref={nodeRef}>
			<Handle
				...
				className="bg-teal-500! rounded-none! w-3 h-3 border-none!"
			/>
			<Card
			...
			>
				<CardHeader
					className={`flex flex-col gap-2 pb-2 space-y-0 border-b ${borderColor} relative`}
				>
					<div className="flex justify-between items-start pr-8">
						<Badge
							variant="outline"
							className={`w-fit bg-teal-950/30 ${textColor} ${borderColor} rounded-none text-xs uppercase`}
						>
						...
						<div className="flex gap-1">
							...
							{data.onShare && (
								<button
									onClick={(e) => {
										e.stopPropagation();
										data.onShare(data, "EMENDA");
									}}
									className={`w-10 h-10 flex items-center justify-center absolute top-1 right-1 hover:bg-white/10 transition-colors z-10 rounded-full ${textColor}`}
								>
					...
					<div className="flex items-start gap-2">
						...
						<CardTitle
							className={`text-sm font-bold uppercase tracking-wider line-clamp-2 ${textColor}`}
							title={data.label}
						>
							{data.label}
						</CardTitle>
				...
				<CardContent className="pt-3 space-y-2">
					<div>
						...
						<p className={`text-sm font-bold mt-0.5 ${textColor}`}>
							R${" "}
							{Number(data._empenhado || data.valor || 0).toLocaleString(
								"pt-BR",
					...
					<div>
						<div className="flex justify-between items-center">
							...
							<p
								className={`text-xs font-bold ${isFantasma ? "text-red-500" : taxa < 30 ? "text-yellow-500" : "text-teal-400"}`}
							>
								{taxa}%
							</p>
						</div>
