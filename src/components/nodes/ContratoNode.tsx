	...
	return (
		<div ref={nodeRef}>
			<Handle
				...
				className="bg-yellow-500! rounded-none! w-3 h-3 border-none!"
			/>
			<Card
			...
			>
				<CardHeader className="flex flex-col gap-2 pb-2 space-y-0 border-b border-yellow-500 relative">
					<div className="flex justify-between items-start pr-8">
						<Badge
							variant="outline"
							className="w-fit bg-yellow-950/30 text-yellow-500 border-yellow-500 rounded-none text-xs uppercase"
						>
						...
						{data.onShare && (
							<button
								onClick={(e) => {
									e.stopPropagation();
									data.onShare(data, "CONTRATO");
								}}
								className="w-10 h-10 flex items-center justify-center absolute top-1 right-1 hover:bg-white/10 transition-colors z-10 rounded-full text-yellow-500"
							>
					...
					<div className="flex items-start gap-2">
						...
						<CardTitle
							className="text-sm font-bold uppercase tracking-wider line-clamp-2 text-yellow-500"
							title={data.label}
						>
							{data.label}
						</CardTitle>
				...
				<CardContent className="pt-4 space-y-2">
					<div>
						...
						<p
							className="text-xs mt-1 text-yellow-400 line-clamp-3"
							title={data.objeto}
						>
							&gt; {data.objeto || "N/A"}
						</p>
