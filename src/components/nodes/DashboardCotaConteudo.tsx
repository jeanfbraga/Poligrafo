...
function DashboardCotaCategoriaGrafico({ categorias }: { categorias: any[] }) {
				...
				<ResponsiveContainer width="100%" height="100%">
					<RechartsPieChart>
						<Pie
							...
							stroke="none"
						>
							{topCategorias.map((_: any, index: number) => (
								<Cell
									key={`cell-${index}`}
				...
				{topCategorias.map((cat: any, index: number) => (
					...
					>
						<div
							...
							style={{
								backgroundColor: CORES_PALETA[index % CORES_PALETA.length],
							}}
						/>
...
function DashboardCotaTopCategorias({
	categorias,
	maxCategoria,
}: {
	categorias: any[];
	...
	return (
		<div>
			...
			<div className="space-y-3">
				{categorias.slice(0, 5).map((cat: any, i: number) => {
...
function DashboardCotaEvolucao({ gastosMensais }: { gastosMensais: any[] }) {
	if (!gastosMensais || gastosMensais.length <= 1) return null;
	const ultimos12 = gastosMensais.slice(-12);
				...
				<ResponsiveContainer width="100%" height="100%">
					<BarChart data={ultimos12}>
						...
						<Bar dataKey="valor" radius={[2, 2, 0, 0]}>
							{ultimos12.map((_: any, index: number) => (
								<Cell
									key={`cell-${index}`}
									fill="#6366f1"
...
function DashboardCotaAvisoTransparencia() {
	return (
...
function DashboardCotaCarregamento({ loading }: { loading: boolean }) {
...
function DashboardCotaErro({ error }: { error?: string }) {
...
function DashboardCotaSucesso({ data }: { data: any }) {
	...
	return (
		<>
			...
			<DashboardCotaCategoriaGrafico categorias={data.gastosPorCategoria} />
			<DashboardCotaTopCategorias
				categorias={data.gastosPorCategoria}
				maxCategoria={maxCategoria}
			/>
			<DashboardCotaEvolucao gastosMensais={data.gastosMensais} />
	...
	return (
		<div className="space-y-6 pb-4">
			<DashboardCotaHeader
				...
				loading={loading}
			/>
			<DashboardCotaAvisoTransparencia />
			<DashboardCotaCarregamento loading={loading} />
			{!loading && <DashboardCotaErro error={data?.error} />}
			{!loading && <DashboardCotaSucesso data={data} />}
		</div>
	);
};
