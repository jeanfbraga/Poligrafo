...
function getCargoBadgeClass(cargo?: string) {
	if (cargo === "Presidente") return "bg-green-500/30 text-green-200 border-green-400";
	if (cargo === "Vice-Presidente" || cargo === "Relator") return "bg-emerald-950/80 text-emerald-300 border-emerald-500";
	return "bg-black text-green-400 border-green-800";
}

function TabTemas({ gruposTematicos, search }: { gruposTematicos: Record<string, FrenteFormatada[]>; search: string }) {
	if (Object.keys(gruposTematicos).length === 0) {
		return (
				...
				const filtrados = term
					? items.filter(
							(i) =>
								...
								i.raw.toLowerCase().includes(term) ||
								(i.sigla && i.sigla.toLowerCase().includes(term)),
					  )
					: items;
...
function TabComissoes({ comissoesFiltradas }: { comissoesFiltradas: ComissaoFormatada[] }) {
			...
			{comissoesFiltradas.map((c, idx) => (
				...
				>
					<div className="flex items-start justify-between gap-2">
						...
						{c.cargo && (
							<span className={`text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 border ${getCargoBadgeClass(c.cargo)}`}>
								{c.cargo}
							</span>
						)}
					</div>
...
function TabFrentes({ frentesFiltradas }: { frentesFiltradas: FrenteFormatada[] }) {
...
function obterComissoesFormatadas(comissoes: any[]): ComissaoFormatada[] {
	return (comissoes || [])
		.filter(Boolean)
...
function obterFrentesFormatadas(frentes: any[]): FrenteFormatada[] {
	return (frentes || [])
		.filter(Boolean)
...
function filtrarFrentesPorTexto(frentes: FrenteFormatada[], search: string): FrenteFormatada[] {
...
function filtrarComissoesPorTexto(comissoes: ComissaoFormatada[], search: string): ComissaoFormatada[] {
	if (!search.trim()) return comissoes;
	const s = search.toLowerCase();
...
function FrentesComissoesDialogHeader({
	nomePolitico,
	partido,
	uf,
	totalComissoes,
	...
	const rotuloPolitico = partido ? `${nomePolitico} (${partido}-${uf})` : nomePolitico;
	const textoComissoes = `${totalComissoes} ${totalComissoes === 1 ? "Comissão" : "Comissões"}`;
	const textoFrentes = `${totalFrentes} ${totalFrentes === 1 ? "Frente" : "Frentes"}`;
...
export default function FrentesComissoesDialog({
	...
	const comissoesFormatadas = useMemo(() => obterComissoesFormatadas(comissoes), [comissoes]);
	const frentesFormatadas = useMemo(() => obterFrentesFormatadas(frentes), [frentes]);
	const gruposTematicos = useMemo(() => agruparFrentesPorTema(frentes || []), [frentes]);
	const frentesFiltradas = useMemo(() => filtrarFrentesPorTexto(frentesFormatadas, search), [frentesFormatadas, search]);
	const comissoesFiltradas = useMemo(() => filtrarComissoesPorTexto(comissoesFormatadas, search), [comissoesFormatadas, search]);
		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="w-[95vw] sm:max-w-3xl max-h-[90vh] sm:max-h-[85vh] flex flex-col p-0 border-2 border-green-500/70 bg-black text-green-400 font-mono shadow-[0_0_35px_rgba(34,197,94,0.2)] rounded-none">
				...
				<div className="flex-1 overflow-y-auto p-3.5 sm:p-5 space-y-5">
					{tab === "temas" && <TabTemas gruposTematicos={gruposTematicos} search={search} />}
					{tab === "comissoes" && <TabComissoes comissoesFiltradas={comissoesFiltradas} />}
					{tab === "frentes" && <TabFrentes frentesFiltradas={frentesFiltradas} />}
				</div>
