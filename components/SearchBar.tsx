...
const formatAutoRef = (p: any): string | undefined => {
    ...
    if (p.casa === 'GOVERNO_ESTADUAL' || p.casa === 'GOVERNADOR') return `GOVERNADOR:${p.uf || 'BR'}:${p.id || p.nome}`;
    if (p.casa === 'PREFEITO' || p.casa === 'PREFEITURA') return `PREFEITO:${p.uf || 'BR'}:${p.id || p.nome}`;
    return undefined;
};
...
export default function SearchBar({
    ...
    const [autocompleteSuggestions, setAutocompleteSuggestions] = useState<typeof congressoIndex>([]);
    ...
    const handleSearchTermChange = (value: string) => {
        ...
        const termoNorm = value.trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
        const matches = congressoIndex.filter((p: any) => {
            const nameMatch = p.nome.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').includes(termoNorm);
        ...
        const uniqueMatches = Array.from(new Map(matches.map((m: any) => [String(m.id || m.nome), m])).values()).slice(0, 8);
    ...
    const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
        if (showAutocomplete && autocompleteSuggestions.length > 0) {
            if (e.key === 'ArrowDown') {
                e.preventDefault();
                setAutocompleteIdx(prev => Math.min(prev + 1, autocompleteSuggestions.length - 1));
                return;
            ...
            <div className="w-full relative max-w-sm mx-auto z-20">
                <div className="flex flex-col gap-3 w-full">
                    ...
                    <div className="relative">
                        ...
                        >
                            <option value="" disabled className="bg-black text-green-500">SELECIONE A ALÇADA</option>
                            <option value="FEDERAL" className="bg-black text-green-400 font-bold">🏛 GOVERNO FEDERAL</option>
                            <option disabled className="bg-black text-green-700">──────────────────</option>
                            <optgroup label="GOVERNO ESTADUAL" className="bg-black text-green-600 uppercase text-xs">
                                {alcadas.filter(e => e.sigla !== 'FEDERAL' && e.sigla !== '_SEP_').map(uf => (
                        ...
                        <div className="pointer-events-none absolute inset-y-0 right-3 flex items-center">
                            <svg className="w-4 h-4 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                        </div>
                    </div>
                    ...
                    {isLoading && onCancel ? (
                        ...
                        <Button
                            variant="cyber"
                            onClick={() => { setShowAutocomplete(false); onSearch(); }}
                ...
                {showAutocomplete && autocompleteSuggestions.length > 0 && !isLoading && (
                    <div className="absolute top-[108px] left-0 w-full border border-green-500/50 bg-black/95 backdrop-blur-sm z-[60] font-mono divide-y divide-green-900/30 shadow-[0_4px_20px_rgba(34,197,94,0.2)]">
                        {autocompleteSuggestions.map((p: any, i: number) => (
                            <button
                                ...
                                className={`w-full text-left px-4 py-3 text-xs uppercase tracking-wider flex items-center justify-between transition-colors ${
                                    i === autocompleteIdx ? 'bg-green-900/40 text-green-400' : 'text-green-500/80 hover:bg-green-950/40 hover:text-green-400'
                                }`}
                                onMouseDown={(e) => {
                                    e.preventDefault();
                            ...
                            >
                                <span className="flex items-center gap-2">
                                    {p.urlFoto ? (
                                        <img src={p.urlFoto} alt={p.nome} className="w-6 h-6 object-cover rounded-sm border border-green-500 shrink-0" />
                                    ) : (
            ...
            <div className="flex w-full items-center h-12 border-none">
                ...
                <div className="relative h-full flex items-center bg-black border border-r-0 border-green-500/50 hover:border-green-400 focus-within:border-green-500 transition-colors">
                    <select
                        ...
                        onChange={(e) => setSelectedUf(e.target.value)}
                        disabled={isLoading}
                        className="h-full bg-green-950/30 border border-green-500 text-green-400 font-mono text-xs tracking-wider uppercase rounded-none appearance-none px-3 pr-8 cursor-pointer focus:outline-none focus:ring-1 focus:ring-green-500 disabled:opacity-50 min-w-[160px]"
                        style={{ WebkitAppearance: 'none' }}
                        aria-label="Selecione a alçada"
                    >
                        <option value="" disabled className="bg-black text-green-500">SELECIONE A ALÇADA</option>
                    ...
                    <div className="pointer-events-none absolute inset-y-0 right-2 flex items-center">
                        <svg className="w-3 h-3 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                    </div>
                </div>
