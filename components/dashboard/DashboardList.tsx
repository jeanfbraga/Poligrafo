  ...
  return (
    <div className="space-y-3">
      {items.slice(0, limit).map((item, i) => {
        const content = (
          <>
            {showRank && <span className="text-green-700 w-5 shrink-0">{i + 1}.</span>}
            <span className="truncate text-amber-400 font-bold">{item.label}</span>
          </>
        );
                  ...
                  <HoverCardContent className="w-80 bg-black/95 text-green-400 font-mono shadow-[0_0_20px_rgba(34,197,94,0.3)] z-50 border-none">
                    <div className="flex justify-between space-x-4">
                      ...
                      <div className="space-y-1 flex-1 min-w-0">
                        <h4 className="text-sm font-bold uppercase tracking-widest leading-tight">{item.profile.nome}</h4>
                        {item.profile.cargo && (
                    ...
                    <div className="mt-4 pt-4 border-t border-green-900/50">
                      <button 
                        ...
                        onClick={() => {
                          const event = new CustomEvent("poligrafo:search", { detail: { nome: item.profile?.nome, id: item.profile?.id } });
