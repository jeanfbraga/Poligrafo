...
export const RaioXGastosNode = ({ data }: { data: any }) => {
    return (
        <>
            <Handle type="target" position={Position.Top} className="!bg-indigo-500 !rounded-none w-3 h-3 !border-none" />
            <Card
                ...
                onClick={(e) => { e.stopPropagation(); data.onOpenDashboard?.(data.nomeVereador); }}
            >
                <div className="absolute inset-0 bg-gradient-to-br from-indigo-950/40 to-black pointer-events-none rounded-none" />
            ...
            <Handle type="source" position={Position.Bottom} className="!bg-indigo-500 !rounded-none w-3 h-3 !border-none" />
        </>
    );
};
