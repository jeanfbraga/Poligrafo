...
const RESOURCE_OPERACOES_NAO_AUTOMATICAS = "6f56b78c-510f-44b6-8274-78a5b7e931f4";
...
async function queryDatastore(resourceId: string, q: string): Promise<OperacaoBNDES[]> {
    try {
        ...
        return json.result.records.map((r: any) => {
            const valor = Number(r.valor_da_operacao_em_reais || r.valor_contratado_reais || r.valor_desembolsado_reais || 0);
            return {
        ...
        console.warn(`[BNDES] Erro ao consultar recurso ${resourceId}:`, e.message || e);
        return [];
    }
}

export async function buscarOperacoesBNDES(cnpjOuNome: string): Promise<OperacaoBNDES[]> {
