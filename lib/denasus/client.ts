...
function classificarTipo(titulo: string): string {
    ...
    if (lower.includes("verificação") || lower.includes("verificacao")) return "Verificação";
    if (lower.includes("monitoramento")) return "Monitoramento";
    if (lower.includes("inspeção") || lower.includes("inspecao")) return "Inspeção";
    return "Outro";
}

export async function listarAtividadesAuditoria(): Promise<AtividadeAuditoria[]> {
    try {
        ...
        if (content.length > 0) {
            const h2s = content.find("h2").filter((_, el) => $(el).find("a").length > 0);
            if (h2s.length > 0) {
                items = h2s;
                strategy = "h2";
            }
        ...
        if (items.length === 0 && content.length > 0) {
            const dts = content.find("dt").filter((_, el) => $(el).find("a").length > 0);
            if (dts.length > 0) {
                items = dts;
                strategy = "dt";
            }
        ...
        items.each((_, el) => {
            const $item = $(el);
            const link = (strategy === "dt" || strategy === "h2") ? $item.find("a") : null;
            const $tituloEl = (link && link.length > 0) ? link : $item.find("h2, h3, a").first();
