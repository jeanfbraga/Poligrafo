...
import congressoIndex from '../data/congresso-index.json';
import { buscarCpfNoTSE, buscarDoadoresTSE, normalizeString, fetchWithTimeout } from '../../app/api/investigar/tse';
import { buscarDetalhesPolitico } from '../../app/api/investigar/scrapers/legislativo';
import { buscarEmendas, buscarDespesasCamara, buscarDespesasSenado } from '../../app/api/investigar/etl_extractors';

import { checkNepotismoCMRJ } from '@/lib/cmrj/nepotismo-client';
import { buscarAeronavesProprietario } from '../anac/client';
import { buscarMunicipalMestre, buscarDespesasMunicipalMestre } from '../../app/api/investigar/municipios/router';
import { buscarDeputadoEstadualRJ, buscarDespesasDeputadoEstadualRJ, buscarPerfilDOCIGP } from '../../app/api/investigar/estados/rj/alerj';
import { buscarDeputadoEstadualSP, buscarDespesasDeputadoEstadualSP } from '../../app/api/investigar/estados/sp/alesp';
import { buscarServidoresCMRJ } from '../../app/api/investigar/estados/rj/camara-rj-client';
import { analisarLoteComInteligencia, analisarEmendasComInteligencia, analisarMalhaOsintComInteligencia } from '../../app/api/investigar/ai_helpers';
import { analyzeGraphNetwork } from '../graph-analysis';

// Helper mockado para simplificar tipagem no momento

import { buscarPolitico, buscarSenador, buscarProjetosLeiCamara, buscarPoliticosCamaraLista, buscarSenadoresLista } from '../../app/api/investigar/scrapers/legislativo';
import { investigarPolitico, buscarReceitasFederais, buscarCartaoCorporativo, buscarViagensFAB } from '../../app/api/investigar/scrapers/osint-fiscal';
import { buscarProcessosDataJud } from '../../app/api/investigar/scrapers/judiciario';
import { expandirMalhaSocietaria, investigarFornecedorNivelHard } from '../../app/api/investigar/scrapers/osint-societario';
import { buscarContratosPNCP, buscarConveniosTransferegov, verificarAeronaveAnac } from '../../app/api/investigar/scrapers/osint-contratos';
import { investigarDiariosOficiais } from '../../app/api/investigar/scrapers/osint-diarios';

import { buscarEmendasPorMunicipio as buscarTransfereGovPorMunicipio, buscarEmendasPorAutor as buscarTransfereGovPorAutor, gerarResumoEmendasPIX, buscarEmendasPorCNPJ } from '../transferegov/client';
import { buscarImoveisMunicipioSupabase } from '../spu/client';
...
import { buscarOperacoesBNDES } from '../bndes/client';
import { buscarInfracoesIbama } from '../ibama/client';
import { buscarCertidaoTCU } from '../tcu/client';
import { buscarEnteSiconfi, consultarIndicadoresLRF } from '../siconfi/client';
import { consultarPNAE, consultarFUNDEB, consultarPNATE } from '../fnde/client';
...
export async function executarInvestigacaoPrincipal(params: any) {
  ...
  // TODO: Injetar lógicas pendentes se necessário
  try {
    {
      // PASSO 1: O Alvo
      ...
      if (!forceRef) {
        ...
        if (cargoParam === 'GOVERNADOR' || cargoParam === 'PREFEITO') {
          ...
          if (tseDados) {
            ...
            if (docTse) {
              candidatosGlobais.push({
                ...
                cargo: cargoParam === 'GOVERNADOR' ? 'Governador de Estado' : 'Prefeito Municipal',
          ...
          const [camaraRes, senadoRes] = await Promise.allSettled([buscarPoliticosCamaraLista(nomeParaBusca, ufScope), buscarSenadoresLista(nomeParaBusca)]);
          clearInterval(delayInterval);
          if (camaraRes.status === 'fulfilled' && camaraRes.value) {
            candidatosGlobais.push(...camaraRes.value.map((c: any) => ({
              ...c,
          ...
          if (senadoRes.status === 'fulfilled' && senadoRes.value) {
            candidatosGlobais.push(...senadoRes.value.map((c: any) => ({
              ...c,
          ...
          // =====================================
          if (candidatosGlobais.length === 0 && ufScope && ufScope !== 'FEDERAL' && ufScope !== 'BR') {
            sendEvent('STATUS', {
              msg: `Buscando fallback na base TSE para cargos Federais em ${ufScope}...`
            });
            const federaisTsePromises = [buscarCpfNoTSE(nomeParaBusca, ufScope, "6").then(tseData => {
              if (tseData) {
                return [{
        ...
        if (candidatosGlobais.length === 0 && !isOnlyFederal) {
          ...
          if (!ufScope || ufScope === 'SP') estaduaisPromises.push(buscarDeputadoEstadualSP(nomeParaBusca));
          if (!ufScope || ufScope === 'RJ') estaduaisPromises.push(buscarDeputadoEstadualRJ(nomeParaBusca));
          ...
          if (ufScope && !['SP', 'RJ'].includes(ufScope)) {
            estaduaisPromises.push(buscarCpfNoTSE(nomeParaBusca, ufScope, "7").then(tseData => {
              if (tseData) {
                return [{
                  id: tseData.documentoPrincipal || tseData.idTse?.toString(),
                  uri: `https://divulgacandcontas.tse.jus.br/divulga/#/candidato/${tseData.anoEleicao}/${tseData.idEleicao}/${ufScope}/${tseData.idTse}`,
        ...
        if (candidatosGlobais.length === 0 && (forceRef?.startsWith('GOVERNADOR:') || correcoesNomes[checkNome]?.autoRef?.startsWith('GOVERNADOR:'))) {
        ...
        const candidatosUnicos = Array.from(new Map(candidatosGlobais.map(c => [c.ref, c])).values());
        ...
        const chaveCacheDeLeitura = refParam ? `${nomeParaBusca}_${refParam}` : nomeParaBusca;
        try {
          if (!isDev) {
            const {
              data: cacheData,
              error: cacheErr
            } = await supabaseAdmin.from('pesquisas').select('grafo_dados').eq('termo_busca', chaveCacheDeLeitura).order('atualizado_em', {
              ascending: false
            }).limit(1).single();
            if (!cacheErr && cacheData && cacheData.grafo_dados && cacheData.grafo_dados.nodes && Array.isArray(cacheData.grafo_dados.nodes) && cacheData.grafo_dados.nodes.length > 0 && cacheData.grafo_dados.partial !== true) {
              sendEvent('STATUS', {
        ...
        if (forceRef && forceRef.startsWith('FEDERAL:CAMARA:')) {
          ...
          if (deputadoBasico) {
            ...
            console.warn(`[CÂMARA] buscarPolitico falhou (timeout/erro) para id=${idRef}. Buscando no índice local...`);
            const localMatch = congressoIndex.find((p: any) => String(p.id) === String(idRef));
            deputadoBasico = {
              id: idRef,
        ...
        } else if (forceRef && forceRef.startsWith('FEDERAL:SENADO:')) {
          const idRef = forceRef.split(':')[2];
          const senadoresAll = await buscarSenadoresLista(nomeParaBusca);
          deputadoBasico = senadoresAll.find(s => String(s.id) === idRef) || null;
          if (!deputadoBasico) {
            console.warn(`[SENADO] buscarSenadoresLista falhou ou ID não achado para id=${idRef}. Buscando no índice local...`);
            const localMatchSenado = congressoIndex.find((p: any) => String(p.id) === String(idRef));
            deputadoBasico = {
              id: idRef,
        ...
        } else if (forceRef && forceRef.startsWith('SP:')) {
        ...
        } else if (forceRef && forceRef.startsWith('RJ:')) {
          const partesRJ = forceRef.split(':');
          const municipioRefRJ = partesRJ.length >= 4 ? partesRJ[2] : 'rio-de-janeiro';
        ...
        } else if (forceRef && forceRef.startsWith('ALERJ:')) {
          const partesAlerj = forceRef.split(':');
          // Formato novo: ALERJ:DEPUTADO_ESTADUAL:{nomeEncoded}:{documento}
          let nomeAlerj = partesAlerj.length >= 3 ? decodeURIComponent(partesAlerj[2]) : (nomeBruto || nomeParaBusca).toUpperCase();
        ...
        } else if (forceRef && forceRef.startsWith('ALESP:')) {
          const partesAlesp = forceRef.split(':');
          // Formato novo: ALESP:DEPUTADO_ESTADUAL:{nomeEncoded}:{documento}
          let nomeAlesp = partesAlesp.length >= 3 ? decodeURIComponent(partesAlesp[2]) : (nomeBruto || nomeParaBusca).toUpperCase();
          ...
          deputadoBasico = {
            ...
            uf: ufGen,
            idLegislatura: 2024,
            casa: cargoGen === 'PREFEITO' ? 'PREFEITURA' : `CAMARA_MUNICIPAL_${ufGen}`
          };
        } else if (forceRef && (forceRef.startsWith('GOVERNADOR:') || forceRef.startsWith('PREFEITO:') || forceRef.startsWith('PRESIDENTE:'))) {
          ...
          deputadoBasico = {
            ...
            casa: cargoTipo === 'PRESIDENTE' ? 'PRESIDENCIA_DA_REPUBLICA' : cargoTipo === 'GOVERNADOR' ? 'GOVERNO_ESTADUAL' : 'PREFEITURA'
          };
      ...
      let detalhes = null;
      if (deputadoBasico.casa === 'CAMARA' && !isNaN(Number(deputadoBasico.id))) {
        try {
      ...
      if (deputadoBasico.casa === 'SENADO') codigoCargoTse = "5";else if (['ALERJ', 'ALESP'].includes(deputadoBasico.casa)) codigoCargoTse = "7";else if (String(deputadoBasico.casa).startsWith('CAMARA_MUNICIPAL')) codigoCargoTse = "13";else if (deputadoBasico.casa === 'GOVERNO_ESTADUAL') codigoCargoTse = "3";else if (deputadoBasico.casa === 'PREFEITURA') codigoCargoTse = "11";else if (deputadoBasico.casa === 'PRESIDENCIA_DA_REPUBLICA') codigoCargoTse = "1";
      const nomeParaTSE = deputadoBasico.nome.replace(/\s*\(.*?\)\s*/g, '').trim();
      let tseResult = await buscarCpfNoTSE(nomeParaTSE, deputadoBasico.uf, codigoCargoTse);
      ...
      if (tseResult && (tseResult.documentoPrincipal || tseResult.cpf)) {
        if (!cpfLimpo || cpfLimpo === "00000000000") {
          cpfLimpo = (tseResult.documentoPrincipal || tseResult.cpf!).replace(/\D/g, '');
      ...
      if (documentoIsCnpj) {
        try {
          ...
          const {
            data,
            error
          } = await supabaseAdmin.from('tse_bens_historico').select('cpf_candidato').ilike('nome_candidato', `%${deputadoBasico.nome}%`).limit(1);
      ...
      sendEvent('STATUS', {
        msg: documentoIsCnpj ? `CNPJ de Campanha capturado (${cpfLimpo}). Usando dados declarados ao TSE...` : `CPF capturado (${cpfLimpo}). Investigando Ficha Limpa, TCU e Processos Judiciais...`
      });
      const tseData = (deputadoBasico as any)._tseResult;
      const fichaPolitico = await investigarPolitico(cpfLimpo, deputadoBasico.nome, deputadoBasico.uf, pessoaId, sendEvent);
      ...
      if (tseData?.patrimonioTotal && tseData.patrimonioTotal > 0) {
        ...
        if (!fichaPolitico.alertasPessoais.some(a => a.includes('[TSE] Patrimônio'))) {
          fichaPolitico.alertasPessoais.push(`[TSE] Patrimônio Declarado (${tseData.anoEleicao || 'Eleição'}): R$ ${ptFmt}`);
        }
      }
      if (documentoIsCnpj && fichaPolitico.patrimonioTotal === 0) {
        fichaPolitico.alertasPessoais.push('[LGPD] Patrimônio pessoal oculto no TSE (Apenas CNPJ de Campanha disponível).');
      }
      let cargoDisplay = 'Político';
      if (deputadoBasico.casa === 'ALERJ' || deputadoBasico.casa === 'ALESP') cargoDisplay = 'Deputado Estadual';else if (String(deputadoBasico.casa).startsWith('CAMARA_MUNICIPAL')) cargoDisplay = 'Vereador Municipal';else if (deputadoBasico.casa === 'CAMARA') cargoDisplay = 'Deputado Federal';else if (deputadoBasico.casa === 'SENADO') cargoDisplay = 'Senador da República';else if (deputadoBasico.casa === 'GOVERNO_ESTADUAL') cargoDisplay = 'Governador';else if (deputadoBasico.casa === 'PREFEITURA') cargoDisplay = 'Prefeito';
      const pessoaNodePayload = {
        ...
        data: {
          ...
          documentoPrincipal: cpfLimpo && cpfLimpo !== '00000000000' ? cpfLimpo : undefined,
      ...
      const isDev = process.env.NODE_ENV === 'development';
      const chaveCacheDeSalvamento = refParam ? `${nomeParaBusca}_${refParam}` : nomeParaBusca;
      if (!isDev) {
        try {
          const {
            data,
      ...
      const isExecutivo = deputadoBasico.casa === 'GOVERNO_ESTADUAL' || deputadoBasico.casa === 'PREFEITURA';
      ...
      if (deputadoBasico.casa === 'PREFEITURA' || String(deputadoBasico.casa).startsWith('CAMARA_MUNICIPAL')) {
        await investigarDiariosOficiais(deputadoBasico.nome, deputadoBasico.uf, pessoaId, sendEvent, supabaseNodes);
      }

      // Prepara contexto normativo comum
      let esferaPolitico = 'FEDERAL';
      if (deputadoBasico.casa === 'ALERJ' || deputadoBasico.casa === 'ALESP') esferaPolitico = 'ESTADUAL';else if (deputadoBasico.casa === 'CAMARA_MUNICIPAL_SP' || deputadoBasico.casa === 'CAMARA_MUNICIPAL_RJ' || String(deputadoBasico.casa).startsWith('CAMARA_MUNICIPAL') || deputadoBasico.casa === 'PREFEITURA') esferaPolitico = 'MUNICIPAL';
      ...
      try {
        if (deputadoBasico.casa === 'PREFEITURA') {
          const nomeMunic = deputadoBasico.uri && deputadoBasico.uri.length > 2 ? deputadoBasico.uri.replace(/-/g, ' ') : deputadoBasico.nome;
          sendEvent('STATUS', {
        ...
        if (transfereGov.length > 0) {
          ...
          const totalCusteio = resumoPix?.valorTotalCusteio || transfereGov.reduce((acc, t) => acc + (t.valorCusteio || 0), 0);
          const totalInv = resumoPix?.valorTotalInvestimento || transfereGov.reduce((acc, t) => acc + (t.valorInvestimento || 0), 0);
          ...
          const tgPayload = {
            ...
            data: {
              ...
              alertas: [`Mapeadas ${numEmendas} transferências especiais diretas (Emendas PIX).`, `Custeio: R$ ${totalCusteio.toLocaleString('pt-BR')}`, `Investimento: R$ ${totalInv.toLocaleString('pt-BR')}`]
            }
          };
          ...
          try {
            ...
            if (!isDev) {
              ...
              if (cacheData?.grafo_dados?.nodes) {
                const cachedTgNode = cacheData.grafo_dados.nodes.find((n: any) => n.type === 'EMENDA_RESUMO' && n.data?.label?.includes('TRANSFEREGOV'));
                if (cachedTgNode) {
                  sendEvent('STATUS', {
        ...
        sendEvent('STATUS', { msg: `Falha na conexão com o TransfereGov (API Offline). Emitindo alerta no Canvas.` });
        const errorPayload = {
          ...
          data: {
            ...
            descricao: "O sistema governamental do Transferegov.br está instável ou fora do ar no momento.",
      ...
      try {
        if (deputadoBasico.casa === 'PREFEITURA') {
          ...
          const spuImoveis = await buscarImoveisMunicipioSupabase(nomeMunic, deputadoBasico.uf);
          if (spuImoveis.length > 0) {
            // Soma o valor total
            const valorTotal = spuImoveis.reduce((acc, i) => acc + (i.valor_imovel || 0), 0);
            const areaTotal = spuImoveis.reduce((acc, i) => acc + (i.area_m2 || 0), 0);
            const spuPayload = {
              ...
              data: {
                ...
                valor: valorTotal > 0 ? `R$ ${valorTotal.toLocaleString('pt-BR')}` : `Área: ${Math.round(areaTotal).toLocaleString('pt-BR')} m²`,
      ...
      if (emendas.length > 0) {
        ...
        const emendasAvaliadasPelaIA = await analisarEmendasComInteligencia(emendas, deputadoBasico.uf, esferaPolitico, deputadoBasico.casa);
        ...
        emendasAvaliadasPelaIA.forEach((emenda: any, i: number) => {
          ...
          if (transfereGov.length > 0) {
            const matchedTg = transfereGov.find(tg => tg.numeroEmenda && (String(tg.numeroEmenda).includes(String(emenda.codigoEmenda)) || String(emenda.codigoEmenda).includes(String(tg.numeroEmenda))));
            if (matchedTg) {
              beneficiario = {
          ...
          const emendaPayload = {
            ...
            data: {
              ...
              objeto: emenda.localidadeDoGasto || emenda.funcao || "Localidade/Função Não Informada",
              ...
              motivo_ia: emenda.motivo_ia ?? `Status Operacional | Pagamento: ${emenda._percentualExecucao}%`,
              classificacao: emenda.classificacao ?? "REGULAR_COM_RESSALVA",
              enquadramento_normativo: emenda.enquadramento_normativo ?? "-",
              fundamentacao_tecnica: emenda.fundamentacao_tecnica ?? "Sem apontamento técnico profundo.",
      ...
      if (fichaPolitico.bensDeclarados && fichaPolitico.bensDeclarados.length > 0) {
        sendEvent('STATUS', {
        ...
        fichaPolitico.bensDeclarados.forEach((bem: any, i: number) => {
          ...
          const bemPayload = {
            ...
            data: {
              ...
              objeto: bem.descricaoDeTalhada || bem.descricao || "Descrição detalhada não informada",
              valor: bem.valor || 0,
              codigo: "Registro TSE"
            }
          };
      ...
      if (deputadoBasico.casa === 'CAMARA') {
        despesasCruas = await buscarDespesasCamara(Number(deputadoBasico.id), sendEvent);
      } else if (deputadoBasico.casa === 'SENADO') {
        despesasCruas = await buscarDespesasSenado(deputadoBasico.id, deputadoBasico.nome, sendEvent);
      } else if (deputadoBasico.casa === 'CAMARA_MUNICIPAL_SP' || deputadoBasico.casa === 'CAMARA_MUNICIPAL_RJ' || String(deputadoBasico.casa).startsWith('CAMARA_MUNICIPAL') || deputadoBasico.casa === 'PREFEITURA') {
        ...
        despesasCruas = await buscarDespesasMunicipalMestre(deputadoBasico.uf, docTce, deputadoBasico.nome, deputadoBasico.uri, deputadoBasico.casa);
        if (deputadoBasico.casa === 'CAMARA_MUNICIPAL_RJ') {
          ...
          const nomeVereadorLimpo = (deputadoBasico as any)._tseResult?.nomeUrna || deputadoBasico.nome;
          ...
          const {
            data: cotaDespesas,
            error: cotaErr
          } = await supabaseAdmin.from('cmrj_despesas').select('*').ilike('vereador_nome', `%${nomeVereadorLimpo.trim()}%`).order('data_despesa', {
            ascending: false
          });
          let totalCota = 0;
        ...
        const perfilDocigp = await buscarPerfilDOCIGP(deputadoBasico.nome, sendEvent);
        if (perfilDocigp) {
          sendEvent('STATUS', {
        ...
        despesasCruas = await buscarDespesasDeputadoEstadualSP(String(deputadoBasico.id), deputadoBasico.nome, sendEvent);
      ...
      await buscarReceitasFederais(cpfLimpo || String(deputadoBasico.id), pessoaId, sendEvent);

      // B2. Buscar Gastos com Cartão Corporativo (CPGF)
      sendEvent('STATUS', {
        msg: "Analisando faturas de Cartão de Pagamento do Governo Federal (CPGF)..."
      });
      await buscarCartaoCorporativo(cpfLimpo || String(deputadoBasico.id), pessoaId, sendEvent, deputadoBasico.casa);
      ...
      await buscarViagensFAB(cpfLimpo || String(deputadoBasico.id), pessoaId, sendEvent, deputadoBasico.casa);

      // C. Expandir Malha Societária
      sendEvent('STATUS', {
        msg: "Expandindo malha societária via BrasilAPI para rastrear blindagem patrimonial..."
      });
      let empresasRelacionadasCNPJs = await expandirMalhaSocietaria(cpfLimpo || String(deputadoBasico.id), pessoaId, sendEvent);
      ...
      try {
        ...
        } = await import('@/lib/services/socio-search');
      ...
      if (empresasRelacionadasCNPJs && empresasRelacionadasCNPJs.length > 0) {
        ...
        const investigacoesEmpresas = empresasRelacionadasCNPJs.map(async cnpjRastreado => {
          sendEvent('STATUS', {
            msg: `[OSINT] Checando contratos federais para o CNPJ: ${cnpjRastreado}`
          });
          await buscarReceitasFederais(cnpjRastreado, `empresa-${cnpjRastreado}`, sendEvent);
      ...
      if (empresasRelacionadasCNPJs && empresasRelacionadasCNPJs.length > 0) {
        ...
        try {
          ...
          const resultadosBNDES = await Promise.allSettled(cnpjsParaBNDES.map(cnpj => buscarOperacoesBNDES(cnpj)));
            ...
            if (res.status === 'fulfilled' && res.value && res.value.length > 0) {
              ...
              const bndesPayload = {
                ...
                data: {
                  ...
                  objeto: ops.slice(0, 3).map(o => `${o.produto || 'N/I'} — ${o.situacao || 'N/I'}`).join(' | '),
                  valor: totalBNDES,
                  codigo: cnpj,
                  ano: ops[0]?.data || 'N/I',
                  score_letalidade: 55,
          ...
          console.warn('[BNDES] Erro ao consultar financiamentos:', errBNDES.message || errBNDES);
        }
      }
        ...
        for (const cnpj of empresasRelacionadasCNPJs) {
          try {
            ...
            if (infracoes && infracoes.length > 0) {
              const valorTotalMultas = infracoes.reduce((acc, inf) => acc + (inf.valor_multa || 0), 0);
              const ibamaPayload = {
                ...
                data: {
                  ...
                  tribunal: 'IBAMA',
                  assunto: infracoes.slice(0, 3).map(i => i.tipo_infracao).join(' | '),
                  score_letalidade: 85,
            ...
            console.warn('[IBAMA] Erro ao consultar autuações ambientais:', errIbama.message || errIbama);
          }
        }
      }
      ...
      if (empresasRelacionadasCNPJs.length > 0) {
        ...
        for (const cnpj of empresasRelacionadasCNPJs) {
          try {
            ...
            if (certidao && certidao.temInfracao) {
              const motivos = [];
              if (certidao.situacaoTcu !== 'NADA_CONSTA') motivos.push(`TCU Inidôneos: ${certidao.situacaoTcu}`);
              if (certidao.situacaoCnj !== 'NADA_CONSTA') motivos.push(`CNJ CNIA: ${certidao.situacaoCnj}`);
              if (certidao.situacaoCeis !== 'NADA_CONSTA') motivos.push(`CGU CEIS: ${certidao.situacaoCeis}`);
              if (certidao.situacaoCnep !== 'NADA_CONSTA') motivos.push(`CGU CNEP: ${certidao.situacaoCnep}`);
              const payload = {
            ...
            console.warn('[TCU] Erro ao buscar certidão para empresa:', errCert.message || errCert);
          }
        }
      }

      // E6. Integração TCE-RS para Federais (compliance fiscal/saúde/educação de origem)
      if (deputadoBasico.casa === 'CAMARA' && deputadoBasico.uf === 'RS' && deputadoBasico.uri) {
        sendEvent('STATUS', {
        ...
        try {
          ...
          } = await import('@/app/api/investigar/estados/rs/tce');
          const docTce = cpfLimpo || String(deputadoBasico.id);
          const tceDespesas = await buscarDespesasMunicipalRS(docTce, deputadoBasico.nome, deputadoBasico.uri, deputadoBasico.casa);
            ...
            tceDespesas.forEach((d, i) => {
              const tcePayload = {
                ...
                data: {
                  ...
                  fornecedor: d.nomeFornecedor || d.descricao || 'Informação do TCE-RS',
      ...
      if (deputadoBasico.casa === 'PREFEITURA' && deputadoBasico.uf) {
        ...
        try {
          const nomeMunicipioAlvo = (deputadoBasico as any)._nomeMunicipio || deputadoBasico.nome;
          const enteSiconfi = await buscarEnteSiconfi(deputadoBasico.uf, nomeMunicipioAlvo);
          if (enteSiconfi) {
            const anoAtual = new Date().getFullYear();
            const indicadores = await consultarIndicadoresLRF(enteSiconfi.cod_ibge, anoAtual);
            if (indicadores) {
              const pctStr = indicadores.percentualDespesaPessoal.toFixed(1);
              const situMsg = indicadores.situacaoLimite === 'EXCEDIDO' ? `[ALERTA CRÍTICO LRF] Limite de gasto com pessoal EXCEDIDO: ${pctStr}% da RCL (limite: ${indicadores.limiteMaximoPercentual}%)` : indicadores.situacaoLimite === 'PRUDENCIAL' ? `[ALERTA LRF] Gasto com pessoal no limite prudencial: ${pctStr}% da RCL` : indicadores.situacaoLimite === 'ALERTA' ? `[AVISO LRF] Gasto com pessoal em nível de alerta: ${pctStr}% da RCL` : `[SICONFI] Gasto com pessoal dentro do limite LRF: ${pctStr}% da RCL`;
              sendEvent('STATUS', {
                msg: situMsg
              });
              if (indicadores.situacaoLimite !== 'NORMAL') {
                ...
                const siconfiPayload = {
                  ...
                  data: {
                    ...
                    score_letalidade: indicadores.situacaoLimite === 'EXCEDIDO' ? 85 : indicadores.situacaoLimite === 'PRUDENCIAL' ? 65 : 45,
          ...
          console.warn('[SICONFI] Erro ao consultar indicadores LRF:', errSiconfi.message || errSiconfi);
        }
        ...
        try {
          const nomeMunicipioAlvo = (deputadoBasico as any)._nomeMunicipio || deputadoBasico.nome;
          const anoAtual = new Date().getFullYear();
          const [pnae, fundeb, pnate] = await Promise.all([consultarPNAE(nomeMunicipioAlvo, deputadoBasico.uf, anoAtual), consultarFUNDEB(nomeMunicipioAlvo, deputadoBasico.uf, anoAtual), consultarPNATE(deputadoBasico.uf, nomeMunicipioAlvo) // PNATE usa uf, municipio
          ]);
          let labelFNDE = [];
          let infoPNAE = pnae.length > 0 ? `PNAE (Merenda): R$ ${(pnae[0].valorFnde || 0).toLocaleString('pt-BR')} para ${pnae[0].totalAlunos} alunos.` : '';
          if (infoPNAE) labelFNDE.push(infoPNAE);
          let infoFUNDEB = fundeb.length > 0 ? `FUNDEB: R$ ${(fundeb[0].valorRepasseEstimado || 0).toLocaleString('pt-BR')} (Est.) para ${fundeb[0].quantidadeMatriculas} matrículas.` : '';
          if (infoFUNDEB) labelFNDE.push(infoFUNDEB);
          let infoPNATE = pnate.length > 0 ? `PNATE (Transporte): Atende ${pnate[0].alunosAtendidos} alunos.` : '';
          ...
          if (labelFNDE.length > 0) {
            const fndePayload = {
              ...
              data: {
                ...
                valor: (pnae[0]?.valorFnde || 0) + (fundeb[0]?.valorRepasseEstimado || 0),
                codigo: 'FNDE',
          ...
          console.warn('[FNDE] Erro ao consultar repasses:', errFnde.message || errFnde);
        }
      }
        ...
        for (const cnpj of empresasRelacionadasCNPJs) {
          try {
            ...
            if (emendasDiretas && emendasDiretas.length > 0) {
              const totalPix = emendasDiretas.reduce((acc, curr) => acc + (curr.valorCusteio || 0) + (curr.valorInvestimento || 0), 0);
              const fndePayload = {
            ...
            console.warn('[TransfereGov] Erro ao buscar emendas para empresa:', errTg.message || errTg);
          }
        }
      }
      ...
      if (deputadoBasico.casa === 'SENADO') cargoTse = '5';else if (['ALERJ', 'ALESP'].includes(deputadoBasico.casa)) cargoTse = '7';else if (String(deputadoBasico.casa).startsWith('CAMARA_MUNICIPAL')) cargoTse = '13';else if (deputadoBasico.casa === 'GOVERNO_ESTADUAL') cargoTse = '3';else if (deputadoBasico.casa === 'PREFEITURA') cargoTse = '11';
      const eleicaoIdTse = ['3', '5', '6', '7'].includes(cargoTse) ? '2040602022' : '2045202024';

      // Se for municipal, precisa do código do município (idUe). Se for estadual, a UF basta.
      const localidadeCodigo = tseDataFollow?.idUe || (deputadoBasico.casa === 'GOVERNO_ESTADUAL' ? deputadoBasico.uf : undefined);
      if (localidadeCodigo) {
        const doadoresCnpj = await buscarDoadoresTSE(deputadoBasico.nome, localidadeCodigo, cargoTse, eleicaoIdTse);
        const doadoresUnicosFornecedores = [...new Set(doadoresCnpj.filter((d: string) => d.length === 14))].slice(0, 15);
        if (doadoresUnicosFornecedores.length > 0) {
          ...
          for (const cnpjDoador of doadoresUnicosFornecedores) {
            ...
            try {
              const resComp = await fetch(`https://compras.dados.gov.br/contratos/v1/contratos.json?cnpj_contratada=${cnpjDoador}`);
              if (resComp.ok) {
                const compJson = await resComp.json();
                const contratos = compJson?._embedded?.contratos || [];
                if (contratos.length > 0) {
                  const valorTotal = contratos.reduce((acc: number, c: any) => acc + (Number(c.valor_inicial) || 0), 0);
                  const nodeDoador = {
      ...
      ? (deputadoBasico as any)._tseResult.doadores : await buscarDoadoresTSE(deputadoBasico.nome, deputadoBasico.uf, cargoTse, eleicaoIdTse);
      ...
      if (Array.isArray(doadores) && doadores.length > 0 && typeof doadores[0] === 'string') {
        // Doadores carregados do buscarDoadoresTSE no formato string de CNPJ/CPF (length === 14)
        cnpjsPNCP.push(...[...new Set(doadores as string[])].filter(d => d.length === 14).slice(0, 5));
      }
      if (cnpjsPNCP.length > 0) {
        const promessasPNCP = cnpjsPNCP.map(cnpj => buscarContratosPNCP(cnpj).then(ct => {
          if (ct.length > 0) contratosPNCPGlobais.push({
            cnpj,
            contratos: ct
          });
        }));
      ...
      if (malhaOsintBuffer.length > 0) {
        ...
        const malhaAvaliada = await analisarMalhaOsintComInteligencia(malhaOsintBuffer, deputadoBasico.uf || 'N/I', esferaPolitico, deputadoBasico.casa);
      ...
      if (despesasCruas.length > 0) {
        ...
        try {
          despesasAvaliadas = await analisarLoteComInteligencia(despesasCruas, deputadoBasico.uf, doadores, esferaPolitico, deputadoBasico.casa);
        } finally {
        ...
        for (let i = 0; i < despesasAvaliadas.length; i++) {
          ...
          if (finalScore >= 85) {
            ...
            const hardData = await investigarFornecedorNivelHard(d.cnpjCpfFornecedor);
            ...
            if (d.motivo_ia) alertasFinais.unshift(`[POLÍGRAFO IA]: ${d.motivo_ia}`);

            // ==========================================
            // [NOVO] DETECÇÃO DE NEPOTISMO (CMRJ)
            // ==========================================
            if (deputadoBasico.casa === 'CAMARA_MUNICIPAL_RJ' && hardData.socios && Array.isArray(hardData.socios)) {
              sendEvent('STATUS', {
              ...
              for (const socio of hardData.socios) {
                const nomeSocio = typeof socio === 'string' ? socio : (socio as any).nome || (socio as any).nome_socio;
                if (nomeSocio) {
                  const nepotismoMatch = await checkNepotismoCMRJ(nomeSocio);
                  if (nepotismoMatch) {
                    const lotacaoStr = nepotismoMatch.lotacao || 'Local Desconhecido';
                    const cargoStr = nepotismoMatch.cargo || nepotismoMatch.vinculo || 'Cargo Desconhecido';

                    // Adiciona o Alerta Crítico
                    alertasFinais.unshift(`🚨 [ALERTA DE NEPOTISMO]: O sócio '${nomeSocio}' atua na CMRJ! Lotação: ${lotacaoStr} | Cargo: ${cargoStr}`);
                    ...
                    const nepotismoNode = {
                      ...
                      data: {
                        ...
                        dataDocumento: nepotismoMatch.data_ingresso || String(new Date().getFullYear()),
                        score_letalidade: 100,
            ...
            if (d.motivo_ia) alertasFinais.push(`[POLÍGRAFO IA INFO]: ${d.motivo_ia}`);
          }

          // -- LAZY LOADING OSINT: AERONAVES (ANAC) + FRETAMENTO DEEP --
          const textoBusca = `${d.tipoDespesa} ${d.nomeFornecedor} ${d.motivo_ia}`.toUpperCase();
          const isFretamento = /TÁXI AÉREO|AERONAVE|FRETAMENTO|CHARTER|VOO FRETADO|LOCAÇÃO.*AERONAVE/.test(textoBusca);
          if (isFretamento || textoBusca.includes('LOCAÇÃO')) {
            ...
            if (dadosAnac && dadosAnac.marca) {
              sendEvent('STATUS', {
                msg: `[OSINT LAZY] Aeronave suspeita detectada. Puxando Dossiê do prefixo ${dadosAnac.marca} na ANAC...`
              });
              alertasFinais.push(`[ANAC/RAB] Aeronave Prefixo ${dadosAnac.marca} localizada. Proprietário: ${dadosAnac.proprietario_nome}. Status: ${dadosAnac.situacao_aeronavegabilidade}.`);
              finalScore += 40;
            }
            ...
            if (isFretamento) {
              ...
              if (cnpjForn.length === 14) {
                ...
                try {
                  const frotaFornecedor = await buscarAeronavesProprietario(cnpjForn);
                  if (frotaFornecedor.length > 0) {
                    ...
                    for (const anv of frotaFornecedor) {
                      ...
                      alertasFinais.push(`[ANAC/RAB] Frota do fornecedor: ${anv.prefixo} (${anv.modelo || 'N/I'})`);
                    }
                  }
                } catch (e) {
                  console.warn('[ANAC DEEP] Erro ao buscar frota do fornecedor:', e);
                }

                // 3. CONFLITO: fornecedor de táxi aéreo é empresa do político?
                if (empresasRelacionadasCNPJs.includes(cnpjForn)) {
                  alertasFinais.push('[CONFLITO GRAVÍSSIMO] A empresa de táxi aéreo/fretamento pertence ao próprio parlamentar ou a sócio direto!');
                  finalScore = 100;
                  sendEvent('STATUS', {
                ...
                if (doadores.includes(cnpjForn)) {
                  alertasFinais.push('[CONFLITO GRAVE] A empresa de táxi aéreo é doadora de campanha do parlamentar! Caracteriza toma-lá-dá-cá em fretamento.');
                  finalScore = 100;
                  sendEvent('STATUS', {
                ...
                try {
                  const resFornQsa = await fetchWithTimeout(`https://brasilapi.com.br/api/cnpj/v1/${cnpjForn}`, {
                    timeout: 4000
                  });
                  if (resFornQsa.ok) {
                    ...
                    const nomeCivilNorm = normalizeString(detalhes?.nomeCivil || '');
                    for (const socio of qsaForn) {
                      const socioNorm = normalizeString(socio.nome_socio || '');
                      if (nomeNorm && socioNorm.includes(nomeNorm)) {
                        alertasFinais.push(`[CONFLITO GRAVÍSSIMO] O político "${deputadoBasico.nome}" é SÓCIO da empresa de táxi aéreo (${empForn.razao_social || cnpjForn})!`);
                        finalScore = 100;
                        break;
                      }
                      if (nomeCivilNorm && nomeCivilNorm.length > 5 && socioNorm.includes(nomeCivilNorm)) {
                        alertasFinais.push(`[CONFLITO GRAVÍSSIMO] O nome civil do político "${detalhes?.nomeCivil}" consta como SÓCIO da empresa de táxi aéreo (${empForn.razao_social || cnpjForn})!`);
                        finalScore = 100;
                        break;
                      }
                    }
          ...
          if (doadores.includes(d.cnpjCpfFornecedor)) {
            alertasFinais.push(`[TSE ALERTA MÁXIMO] Conflito de Interesse! Este fornecedor financiou a campanha do político em 2022.`);
          ...
          if (finalScore >= 70) nivelDisplay = 'ALTO';else if (finalScore >= 50) nivelDisplay = 'MEDIO';
          ...
          const despesaPayload = {
            ...
            data: {
              ...
              risco: {
                ...
                fundamentacao_tecnica: d.fundamentacao_tecnica ?? "Sem avaliação técnica específica associada.",
                ...dadosSociais
              }
            }
          };
          ...
          if (i > 0 && i % 5 === 0 && dbSearchId && !isDev) {
            try {
              supabaseAdmin.from('pesquisas').update({
                grafo_dados: {
                  timestamp: new Date().toISOString(),
                  nodes: supabaseNodes,
                  escopo: deputadoBasico?.casa || 'GLOBAL',
              ...
              }).eq('id', dbSearchId).then(); // Pass through assíncrono para não travar o loop
            } catch (e) {}
          }
        }
      } else {
      ...
      try {
        ...
        if (!isDev) {
          if (dbSearchId) {
            const {
              error
            } = await supabaseAdmin.from('pesquisas').update({
              grafo_dados: {
                timestamp: new Date().toISOString(),
                nodes: supabaseNodes,
                escopo: deputadoBasico?.casa || 'GLOBAL',
            ...
            const {
              error
            } = await supabaseAdmin.from('pesquisas').upsert({
              termo_busca: chaveCacheDeSalvamento,
              cpf_raiz: supabaseNodes.find(n => n.type === 'PESSOA')?.data?.cpf?.replace(/\D/g, '') || null,
              grafo_dados: {
                timestamp: new Date().toISOString(),
                nodes: supabaseNodes,
                escopo: deputadoBasico?.casa || 'GLOBAL',
          ...
          console.log("[Supabase Skipping] Ambiente de desenvolvimento detectado. Sincronização com base de inteligência desativada.");
        }
      } catch (e) {
      ...
      try {
        sendEvent('STATUS', { msg: "Executando matemática avançada de grafos..." });
        const metrics = analyzeGraphNetwork(supabaseNodes);
        sendEvent('GRAPH_ANALYSIS_SCORES', metrics);
      } catch (e) {
        console.error("[Graphology] Falha ao analisar e enviar scores do grafo:", e);
      }
