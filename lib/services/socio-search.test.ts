...
describe('buscarEmpresasDoSocio', () => {
    ...
    it('deve retornar a lista de empresas quando a Casa dos Dados responde com sucesso', async () => {
        const mockCasaDosResponse = {
            data: {
                cnpj: [
                    { cnpj: '12345678000199', razao_social: 'EMPRESA TESTE 1', situacao_cadastral: 'Ativa', cnae_descricao: 'Consultoria' },
                    { cnpj: '98765432000188', razao_social: 'EMPRESA TESTE 2', situacao_cadastral: 'Ativa', cnae_descricao: 'Tecnologia' }
                ]
            }
        };
    ...
    it('deve retornar empresas do fallback DuckDuckGo quando Casa dos Dados bloqueia e DuckDuckGo encontra CNPJs válidos', async () => {
        ...
        mockFetch.mockResolvedValueOnce({
            ok: true,
            text: async () => '<div>Resultado para 12.345.678/0001-99 empresa xyz</div>'
        });
