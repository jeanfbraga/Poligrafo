...
import { buscarPoliticosCamaraLista, buscarSenadoresLista, buscarPolitico } from '../../app/api/investigar/scrapers/legislativo';
...
export async function buscarCandidatosEmCascata(params: IdentificacaoParams) {
    const { nomeParaBusca, nomeBruto, ufScope, cargoParam, ufParam, forceRef, correcoesNomes, sendEvent } = params;
    let candidatosGlobais: any[] = [];
    let hasApiError = false;

    if (cargoParam === 'GOVERNADOR' || cargoParam === 'PREFEITO') {
        sendEvent('STATUS', { msg: `Buscando ${cargoParam} diretamente na base eleitoral (TSE)...` });
        ...
        sendEvent('STATUS', { msg: `Buscando na esfera Federal${ufScope && ufScope !== 'FEDERAL' ? ` (filtrando por ${ufScope})` : ''}...` });
        
        let secondsElapsed = 0;
        const delayInterval = setInterval(() => {
            secondsElapsed += 5;
            if (secondsElapsed === 5) {
                sendEvent('STATUS', { msg: `Aguardando resposta dos servidores da Câmara dos Deputados...` });
            } else if (secondsElapsed === 15) {
                sendEvent('STATUS', { msg: `A API oficial da Câmara está lenta hoje, forçando a conexão...` });
            } else if (secondsElapsed === 25) {
                sendEvent('STATUS', { msg: `Ainda aguardando resposta governamental (tentativa final)...` });
            }
        }, 5000);
        ...
        if (camaraRes.status === 'fulfilled' && camaraRes.value) {
            candidatosGlobais.push(...camaraRes.value.map((c: any) => ({ ...c, ref: `FEDERAL:CAMARA:${c.id}`, cargo: 'Deputado Federal' })));
        ...
        if (senadoRes.status === 'fulfilled' && senadoRes.value) {
            candidatosGlobais.push(...senadoRes.value.map((c: any) => ({ ...c, ref: `FEDERAL:SENADO:${c.id}`, cargo: 'Senador da República' })));
        ...
        if (candidatosGlobais.length === 0 && ufScope && ufScope !== 'FEDERAL' && ufScope !== 'BR') {
            sendEvent('STATUS', { msg: `Buscando fallback na base TSE para cargos Federais em ${ufScope}...` });
    ...
    if (candidatosGlobais.length === 0 && !isOnlyFederal) {
        sendEvent('STATUS', { msg: 'Não encontrado na esfera Federal. Buscando na esfera Estadual (ALESP e ALERJ)...' });
    ...
    if (candidatosGlobais.length === 0 && (forceRef?.startsWith('GOVERNADOR:') || correcoesNomes[checkNome]?.autoRef?.startsWith('GOVERNADOR:'))) {
        ...
        sendEvent('STATUS', { msg: `Buscando Governador "${nomeGov}" na base eleitoral TSE (${ufGov})...` });
    ...
    if (candidatosGlobais.length === 0 && !isOnlyFederal) {
        sendEvent('STATUS', { msg: 'Buscando na malha Municipal Master (Prefeitos e Vereadores)...' });
...
export async function identificarCandidatoPorRef(
    ...
    if (forceRef.startsWith('FEDERAL:CAMARA:')) {
    ...
    } else if (forceRef.startsWith('GOVERNADOR:') || forceRef.startsWith('PREFEITO:') || forceRef.startsWith('PRESIDENTE:')) {
        ...
        sendEvent('STATUS', { msg: `Buscando ${cargoTipo} "${nomeGov}" na base eleitoral TSE...` });
