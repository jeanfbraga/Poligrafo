...
const req = http.get('http://localhost:3000/api/investigar?nome=davi+alcolumbre&ref=FEDERAL:SENADO:3830&limit=10', (res) => {
    ...
    res.on('end', () => {
        ...
        if (cacheHitDetected) {
            ...
            console.log('⚠️ Busca nova (Não usou cache do grafo 24h). Verifique se puxou dados do Cache das bases.');
        }
        process.exit(0);
    });
});
